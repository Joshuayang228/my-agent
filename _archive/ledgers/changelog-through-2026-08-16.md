## Fixed — 安全审计 v5（2026-08-16）

> **归档快照**：截至 2026-08-16 的完整变更流水。本文件保留原始信息，不作为当前事实源；当前状态请回到项目活跃文档。

- 修复设置 IPC 将解密后的 API Key 和 MCP 环境变量下沉到 Renderer 的高风险问题；设置页只显示“已安全保存”状态。
- 连接测试可由主进程使用已保存 Key，且未修改 Key 时自动保存不会覆盖原凭据。
- MCP 配置统一做结构、资源上限、重复 ID 和 env secret 恢复校验；启用/修改 MCP 与切换 `full-access` 增加主进程原生确认。
- 同步安全审计报告、方法论当前实现矩阵及权限/运行时/记忆模块卡。
...
### Fixed — 安全审计 v4（2026-08-15）

- 危险命令匹配统一按小写规范化，补齐 Windows 删除、编码 PowerShell、关机、磁盘分区和破坏性 Git 清理的 bypass-immune 回归边界。
- 文件写入后的 TypeScript 验证改用 `npx --no-install`，缺少本地工具时不会隐式联网安装。
- 方法论新增当前实现审计矩阵，校准 M08/M09/M11/M14/M21–M32 的真实代码入口、测试证据和未实现边界。
