...
> **已降级（2026-07-30）**：不再镜像维护类型全文。  
> **真相**：`src/shared/types.ts` + preload + `electron/main/ipc/*`（IPC 四处同步，见根目录 `CLAUDE.md`）。  
> 产品横切影响面看 [`modules/`](./modules/)。下文为历史快照，可能过时。

## 约定

- ~~接口变更时必须同步更新此文档~~ → 改类型与 IPC 时同步代码三处 + `vite-env.d.ts`，不必更新本文
- 改接口前以 `src/shared/types.ts` 与模块卡为准
