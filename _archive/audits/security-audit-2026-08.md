...
> **归档快照**：2026-08-16 安全审计及当时验证证据。本文件保留原始信息，不作为当前事实源；当前状态请回到项目活跃文档。
...
6. OS 级 Shell 强隔离与 Python 嵌入沙箱不属于当前个人桌面 Agent 威胁模型，已由 DEC-037 明确不做；Shell 解释器语义、symlink TOCTOU 和子进程派生作为已披露剩余风险，由现有应用层 fail-closed 边界缓解。只有威胁模型触发条件变化时才重新立项独立受限 Runner。
