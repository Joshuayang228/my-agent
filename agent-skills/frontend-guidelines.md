...
- **三类工作域**：`基础` 生产设计令牌、图标与视觉资产、基础组件；`产品体验` 组合 Chat、人物世界、记忆、设置、工作区和业务状态；`Agent 实验` 负责对话、模型和工具试验。
- **两层设计边界**：设计基础提供可复用的样式、组件和交互能力；产品体验只负责业务语义、数据、页面组合和流程，不另造基础组件。
...
- **反向升级**：仅一个业务场景需要的结构留在产品体验；多个场景反复需要且可脱离业务复用的能力，先回流基础侧再登记。
- **壳**：产品内全页 + 一级工作域导航；工作台内部只允许筛选和状态切换，不再嵌套承担页面跳转职责的二级导航。
