...
- **Tab 事实表**：设计语言只放颜色 / 主题 / 圆角动效；图标与视觉只放可复用图标；基础组件只放脱离业务的组件故事；人物世界内部并列展示朋友圈 / 物什 / 名册 / 角色架；工作区 P0 固定为审阅 / 浏览器 / 文件 / 终端 / 侧边聊天，预览属于文件功能，不加入任务进度或完成状态；正式工作区须经 P0 验收后另行回流。同类型多实例必须有独立稳定标识。每次新增或迁移 Tab，必须同步施工合同、对应注册表和 E2E。
...
- **Hover / focus / active / loading / 编辑 / 确认状态不得改变布局占位**：状态切换前后，相关元素及其周围内容的 `width`、`height`、`x`、`y` 和换行结果必须保持稳定。
- **操作槽必须预留**：默认状态没有显示编辑、删除、关闭等操作时，也必须预留这些按钮的完整宽度和高度。可以用 `opacity`、透明文字、覆盖层或 `visibility` 隐藏内容，但不得通过卸载元素、`display: none`、`width: 0` 或动态改变 padding 来释放空间。
- **同一位置原位替换**：日期与编辑 / 删除、添加与取消添加等互斥内容必须共用固定尺寸的容器；文字变更不能导致按钮或邻近正文横向、纵向跳动。
- **验证要求**：E2E 或浏览器验收必须比较状态切换前后的几何尺寸和相对位置，至少覆盖默认、hover / focus、取消和保存 / 确认路径；长文本、窄屏和深浅主题也不能破坏占位稳定性。
...
## Playground 量化设计与交互审计

本节把外部设计 / 交互审计规则合并为本项目唯一前端审计入口；不再创建重复的“审美体检”或“交互巡检” Skill。

### 适用范围与边界

适用于：新增或修改 Playground Foundation 故事、产品体验候选、页面组合和交互状态。

- Playground 是隔离候选，不得默认读写真实会话、设置、LLM、MCP 或生产数据。
- 未经用户明确确认，不得把候选方案回流正式 Chat、Settings、Sidebar 或人物世界。
- 只改未通过项；不整体重做；不删除代码或视觉元素而不说明原因。
- 品牌色、Logo、字体族属于品牌资产，修改前必须征得用户确认。

### 固定审计流程

1. **先盘点**：列出主要页面 / Story、入口组件、异步请求、危险操作和公共样式。
2. **读取事实源**：检查 `src/index.css`、主题变量、Foundation 注册表、正式组件和相关施工合同。
3. **量化审计**：每项输出“通过 / 不通过 / 不适用”，附绝对文件路径、行号、具体值和触发场景。
4. **按一类问题修改**：一次只改一类问题，改完立刻运行相关测试和检查。
5. **完整验收**：走默认态、空态、错误态、窄宽、深色 / 浅色、键盘和失败路径。

### 设计令牌与裸值

审计必须盘点颜色、字号、字重、间距、圆角、阴影、动效和断点的变量 / Tailwind 值 / 裸值。

- 页面级间距优先使用 8 的倍数；控件内部允许 4 的倍数半档；1–3px 只能用于边框、分隔线或视觉校正并说明原因。
- 同一语义必须只有一个变量来源；主行动色按页面收拢为一个，`success` / `warning` / `danger` / `--companion-accent-warm` 属于不同语义，不算重复主色。
- 矩形控件最多两档语义圆角；头像、状态点、圆形图标按钮和胶囊标签另计。
- 核心页面层级不超过三档；辅助文字可以是功能层，但不得制造新的主视觉层级。
- 一个视觉块只回答一个问题，信息按重要性分层：主任务先于辅助信息，次要元数据进入详情或按需状态，不得把名称、说明、状态和操作无差别堆在同一块。
- 新规范落地时必须回头修旧 Playground 页，不能只约束新增代码。同类对象用独立卡片，不把多条记录塞进一张大卡或用 divide-y 内部分割行冒充层级；文字链操作优先改成图标按钮。
- Playground UI 改动交付前，至少扫一遍设置、记忆、Skills、MCP、权限和当前相关页，确认没有新的“一块回答多个问题”回归。
- 优先把颜色、圆角、阴影、页面级间距和 Foundation 共享 padding 收进 CSS 变量；协议值、SVG 坐标和有明确业务含义的一次性尺寸不强行抽象。
- 发现阴影、渐变或分隔线时先说明用途；无法证明用途的进入删除候选，经过截图 / 主题对比后再删，不能直接批量删除。

### 交互巡检清单

- 数据视图必须有加载、空、错误三态；空态说明下一步，不只写“暂无数据”。
- 点击后 100ms 内必须有视觉反馈；超过 1 秒显示处理中线索；长任务显示真实阶段、进度或可取消入口，不伪造 ETA。
- 提交类操作必须防重复提交；危险操作优先真实可撤销，若涉及回收站 / 软删除等后端契约，先给方案并获确认。
- 前置条件不满足时禁用并说明缺什么；危险操作与常用操作分组，不肩并肩抢主视觉。
- 互斥用单选，多选用复选框，立即生效用开关；选项未超一屏时不强行使用下拉。
- 弹窗逐个判断是否必要；能用页内提示解决的，不新增弹窗。
- 按钮文案写清动作和后果；错误文案面向用户并给补救办法，不暴露状态码、字段名或内部路径。
- 同类按钮、输入框、卡片、Toast、Dialog 必须复用 Foundation 实现、token 和无障碍行为。
- 图片提供 `alt` 和适合尺寸；文本截断有省略方案；200% 缩放、键盘焦点和窄宽布局可用。
- 有图表时标注关键数字、单位、时间范围和空态；无图表时标记“不适用”。

### 审计报告格式

```text
现状盘点：页面 / Story、设计变量、异步点、危险操作
逐项审计：通过 / 不通过 / 不适用 + 文件行号 + 数值证据
已修复：现象 + 判据 + 改法 + 前后具体值 + 验证
遗留问题：需要设计或产品决策的事项 + 建议方案
```

### 交付门禁

- 不接受“先做简版”；范围过大时在动手前缩小范围，并写明明确不做项。
- 不留 TODO 占位、假数据空函数或只有界面没有逻辑的正式按钮；Playground fixture 必须明确标注为样张 / 隔离实验。
- 不用“感觉更好看了”作为结论；必须有数值、代码、截图或交互证据。


### 交互审计证据与修复规则

开始巡检前，先列出主要用户流程、所有异步请求点、持久化点和危险操作点；不能只按组件文件逐个看，因为交互问题通常发生在状态流转和调用链上。

每条不通过结论必须按以下格式记录：

```text
现象：用户在什么场景看到或触发了什么行为
判据：违反了哪条规则，证据是什么（文件、行号、状态或时序）
改法：最小完整修复，以及需要回归的成功 / 失败路径
```

交互状态要求：

- 数据视图的加载、空、错误三态必须分别可达；错误态要说明用户能做什么，空态要提供下一步动作。
- 提交、保存、发送、创建和连接类操作在第一次点击后立即进入 pending，并在请求完成、失败或取消前阻止重复提交；不能只依赖按钮文案变化。
- 异步操作超过 1 秒必须显示真实进行中状态；接近 10 秒或更久时显示当前阶段、可取消入口或明确的等待原因，不伪造百分比和预计时间。
- 任何删除、清空、断开或覆盖操作都必须明确后果；需要撤销或回收站时，先确认后端软删除 / 恢复契约，再实施真实可用的撤销，不做只有样式的假 Toast。
- 前置条件不满足时优先禁用操作并说明缺失条件；不要让用户点击后才得到可预见的校验错误。
- 危险操作与常用操作必须在布局和视觉层级上分组，危险按钮不能与主行动按钮并排同色同重。
- 互斥选项用单选或 segmented control，多选用复选框，立即生效的二值设置用开关；选项未超过一屏时不强行使用下拉菜单。
- 每个弹窗都要回答“如果不弹，页内提示是否足够”；能用页内提示完成的，不新增对话框。确认框不能替代撤销机制。
- 按钮文案必须包含动作和必要后果，例如“删除这 3 条记录”，不能使用脱离上下文的“确定”。错误文案面向用户，至少包含原因和补救动作，不暴露状态码、数据库字段、堆栈或内部路径。
- 同一问题只询问一次；用户上次的安全选择、筛选条件、视图偏好或非敏感输入应在有明确作用域和隐私边界时恢复为默认值。

修复约束：

- 一次只修改一类交互问题，改完立即验证对应路径；不得同时改状态机、视觉层和数据契约后再猜是哪一处生效。
- 涉及 IPC、存储、回收站、软删除、恢复或权限契约时，先补完整方案和影响范围，再获得用户确认后实施。
- 交互审计不以“元素存在”作为通过；必须验证点击、键盘、加载、空、错误、取消、重复点击和恢复路径。
- Playground 的异步和危险操作默认使用隔离 fixture，并明确标注样张；不得把假成功状态带入正式产品。
