...
const CORE_PROMPT_ASSETS: readonly (PromptAsset & { content?: string })[] = [
  dynamicAsset({
    ...
    slots: [
      ...
      { name: 'companionResponseNote', source: 'settings.companionResponseNote', lifecycle: '每轮主对话 / 召唤；workspace 排除' },
  ...
  dynamicAsset({
    ...
    role: 'system',
    desc: '保留旧配置中的 System 补充；新伙伴设置的相处说明使用独立字段，不覆盖此内容。',
  ...
  dynamicAsset({
    id: PROMPT_KEYS.companionResponseNote,
    ownership: 'user',
    name: '用户相处偏好',
    category: 'system',
    purpose: '主对话和召唤的 L3 回应偏好；workspace 排除',
    role: 'companion',
    desc: '用户相处说明作为独立上下文，不覆盖身份或工具权限；空值不注入。',
    sourcePath: 'electron/main/agent/prompt-builder.ts',
    preview: '正文来自 settings.companionResponseNote；不在内置资产目录复制用户正文。',
    slots: [{ name: 'companionResponseNote', source: 'settings.companionResponseNote', lifecycle: '每轮主对话 / 召唤' }],
  }),
  dynamicAsset({
