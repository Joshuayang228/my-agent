...
import { Brain, Check, ChevronRight, CircleHelp, Cloud, Database, Download, Eye, Heart, KeyRound, Link2, LockKeyhole, Palette, Plug, Save, Settings2, ShieldCheck, SlidersHorizontal, Sparkles, Upload, UserRound, Wrench, Activity, Gauge, Plus, Server, ListChecks, ArrowUp, ArrowDown, GripVertical, RefreshCw, Trash2, X } from 'lucide-react'
...
function ModelPage({ modelStatus, onModelStatusChange, selectedProvider }: { modelStatus: 'idle' | 'success' | 'error'; onModelStatusChange: (status: 'idle' | 'success' | 'error') => void; selectedProvider: string; onProviderChange: (provider: string) => void }) {
  ...
  /**
   * 背景：获取动作在卡片头部，折叠时也能触发；设计意图：按连接保存候选和请求，成功后只供下拉选填。
  ...
  const setPreview = (state: 'empty' | 'one' | 'two') => { fetchTimers.current.forEach((timer) => window.clearTimeout(timer)); fetchTimers.current.clear(); setFetchStates({}); setFetchedModelsByConnection({}); setModelDrafts({}); setPreviewState(state); if (state === 'empty') { setConnections([]); setExpandedId(null); setRoutes({ primary: [], auxiliary: [], image: [] }); return }; const first = createConnection(firstProvider); if (state === 'one') { setConnections([first]); setExpandedId(first.id); setRoutes({ primary: [{ connectionId: first.id, modelId: 'gpt-4o', enabled: true }, { connectionId: first.id, modelId: 'gpt-4o-mini', enabled: true }], auxiliary: [{ connectionId: first.id, modelId: 'gpt-4o-mini', enabled: true }], image: [{ connectionId: first.id, modelId: 'image-model-id', enabled: true }] }); return }; const second = createConnection(allProviders.find((item) => item.providerId === 'openrouter') ?? allProviders[1], 1); second.name = '国际流动'; second.source = 'relay'; second.models = [{ id: 'deepseek-chat', enabled: true, visibleInPicker: true, purpose: 'unused', image: 'unknown', tools: 'unknown' }, { id: 'deepseek-reasoner', enabled: true, visibleInPicker: true, purpose: 'unused', image: 'unknown', tools: 'unknown' }]; setConnections([first, second]); setExpandedId(first.id); setRoutes({ primary: [{ connectionId: first.id, modelId: 'gpt-4o', enabled: true }, { connectionId: second.id, modelId: 'deepseek-chat', enabled: true }], auxiliary: [{ connectionId: second.id, modelId: 'deepseek-reasoner', enabled: true }], image: [{ connectionId: first.id, modelId: 'image-model-id', enabled: true }] }) }
    ...
    <div className="space-y-4" data-testid="settings-candidate-section-model">
      ...
      <SettingCard testId="settings-candidate-model-connections"><div className="flex items-start justify-between gap-3"><div><h3 className="text-[13px] font-medium" style={{ color: 'var(--text-primary)' }}>连接与模型清单</h3><p className="mt-1 text-[11px]" style={{ color: 'var(--text-muted)' }}>{connections.length ? `${connections.length} 个连接入口；模型用途在上方单独安排。` : '还没有连接，添加后再维护模型清单。'}</p></div><button type="button" onClick={openAdd} className="inline-flex shrink-0 items-center gap-1 rounded-[var(--radius-md)] border px-3 py-1.5 text-[11px] font-medium" style={{ borderColor: 'var(--accent)', color: 'var(--accent-fg)' }} data-testid="settings-candidate-model-add"><Plus size={13} />添加连接</button></div>{connections.length === 0 && <div data-testid="settings-candidate-model-empty" className="mt-5 border-t pt-5 text-center" style={{ borderColor: 'var(--border-subtle)' }}><div className="text-[13px] font-medium" style={{ color: 'var(--text-primary)' }}>还没有连接入口</div><p className="mt-1 text-[11px]" style={{ color: 'var(--text-muted)' }}>添加连接后，在连接下维护多个模型，再到上方安排用途。</p></div>}<div className="mt-4 space-y-2">
        {connections.map((connection) => {
          ...
          return <div key={connection.id} className="min-w-0 rounded-[var(--radius-md)] border" style={{ borderColor: expanded ? 'var(--accent)' : 'var(--border-subtle)' }} data-testid={`settings-candidate-model-profile-${connection.id}`}>
            <div className="flex flex-wrap items-center gap-2 px-3 py-3" data-testid={`settings-candidate-connection-header-${connection.id}`}>
              <button type="button" aria-expanded={expanded} aria-controls={`connection-details-${connection.id}`} onClick={() => setExpandedId(expanded ? null : connection.id)} className="flex min-w-0 flex-1 items-center gap-2 text-left">
                ...
                <ChevronRight size={14} className={`shrink-0 transition ${expanded ? 'rotate-90' : ''}`} style={{ color: 'var(--text-muted)' }} />
              </button>
            ...
            {expanded && <div id={`connection-details-${connection.id}`} className="border-t px-3 py-3" style={{ borderColor: 'var(--border-subtle)' }}>
              ...
              <div className="mt-3 space-y-2">
                ...
                {connection.models.map((model) => <div key={model.id} className="flex min-w-0 items-center gap-2 rounded-[var(--radius-sm)] border px-2.5 py-2" style={{ borderColor: 'var(--border-subtle)' }}>
                  ...
                  <span className="shrink-0 text-[10px]" style={{ color: model.enabled ? 'var(--success)' : 'var(--text-muted)' }}>{model.enabled ? '已启用' : '已停用'}</span>
                  <button type="button" role="switch" aria-checked={model.enabled} aria-label={`${model.id}${model.enabled ? '已启用' : '已停用'}`} onClick={() => updateModel(connection.id, model.id, { enabled: !model.enabled })} className="rounded p-1"><Check size={12} style={{ color: model.enabled ? 'var(--success)' : 'var(--text-muted)' }} /></button>
              ...
              <div className="mt-3 flex min-w-0 items-center gap-2">
                ...
                {fetchState === 'success' && fetchedModels.length > 0 && <select aria-label={`选择已有模型 ${connection.name}`} title="选择已有模型" value="" onChange={(event) => setModelDrafts((current) => ({ ...current, [connection.id]: event.target.value }))} className="theme-input h-8 w-8 shrink-0 rounded-[var(--radius-sm)] border text-[10px] outline-none">
                  <option value="" hidden />
                  {fetchedModels.map((modelId) => <option key={modelId} value={modelId} disabled={connection.models.some((model) => model.id === modelId)}>{modelId}{connection.models.some((model) => model.id === modelId) ? '（已添加）' : ''}</option>)}
                </select>}
