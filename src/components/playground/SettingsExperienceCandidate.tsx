...
function ExtensionsPage() {
  ...
  return <div className="space-y-4" data-testid="settings-candidate-section-extensions">
    <CandidatePageHeader icon={<Plug size={14} />} title="Skills / MCP" description="管理 Skills 与 MCP 连接。" />
    <div className="flex gap-4 border-b" role="tablist" aria-label="扩展类型" data-testid="settings-candidate-extensions-tabs">
      <button type="button" role="tab" aria-selected={activeTab === 'skills'} onClick={() => setActiveTab('skills')} className="border-b-2 px-0.5 py-2 text-[11px] transition" style={{ borderColor: activeTab === 'skills' ? 'var(--accent)' : 'transparent', color: activeTab === 'skills' ? 'var(--text-primary)' : 'var(--text-muted)' }} data-testid="settings-candidate-extensions-tab-skills" data-selected={activeTab === 'skills' ? 'true' : undefined}>Skills</button>
      <button type="button" role="tab" aria-selected={activeTab === 'mcp'} onClick={() => setActiveTab('mcp')} className="border-b-2 px-0.5 py-2 text-[11px] transition" style={{ borderColor: activeTab === 'mcp' ? 'var(--accent)' : 'transparent', color: activeTab === 'mcp' ? 'var(--text-primary)' : 'var(--text-muted)' }} data-testid="settings-candidate-extensions-tab-mcp" data-selected={activeTab === 'mcp' ? 'true' : undefined}>MCP</button>
    </div>
