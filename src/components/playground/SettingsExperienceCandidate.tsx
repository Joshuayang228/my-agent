  ...
  return <div className="space-y-4" data-testid={`settings-candidate-section-${mode}`}>
    ...
    {mode === 'skills' && <>
      ...
      {skillsState === '详情' ? <SettingCard testId="settings-candidate-skill-detail">
        <div className="mb-4 flex items-center justify-between gap-3"><button type="button" onClick={() => setSkillsState('单个')} className="text-[11px]" style={{ color: 'var(--text-secondary)' }} data-testid="settings-candidate-skill-back">返回 Skills</button><CandidateSwitch checked={skillsEnabled} compact label="启用文件整理助手" description="" onChange={setSkillsEnabled} testId="settings-candidate-skills-enabled" /></div>
        <h3 className="text-[15px] font-semibold" style={{ color: 'var(--text-primary)' }}>文件整理助手</h3><p className="mt-1 text-[11px]" style={{ color: 'var(--text-muted)' }}>帮助 Agent 归纳文件、提取重点并整理结果。</p>
        ...
        <div className="mt-4 border-t pt-3 text-[10px]" style={{ borderColor: 'var(--border-subtle)' }}><div className="mb-2 text-[11px] font-medium">使用范围</div><div className="grid gap-2 sm:grid-cols-3"><div>触发：用户请求整理文件时</div><div>工具：读取文件、写入结果</div><div>确认：写入前需要确认</div></div></div>
      </SettingCard> : <SettingCard testId="settings-candidate-skills-list"><div className="divide-y" style={{ borderColor: 'var(--border-subtle)' }}>{(skillsState === '多个' ? ['文件整理助手', '网页研究助手', '代码审查助手'] : ['文件整理助手']).map((name) => <div key={name} className="flex items-center justify-between gap-3 px-1 py-2" data-testid={name === '文件整理助手' ? 'settings-candidate-skill-file-organizer' : undefined}><span className="text-[13px] font-medium" style={{ color: 'var(--text-primary)' }}>{name}</span><CandidateSwitch checked={name === '文件整理助手' ? skillsEnabled : false} compact label={name} description="" onChange={name === '文件整理助手' ? setSkillsEnabled : () => undefined} testId={name === '文件整理助手' ? 'settings-candidate-skills-enabled' : 'settings-candidate-skill-toggle-' + name} /></div>)}</div></SettingCard>}
    </>}
