...
function ExtensionsPage() {
  ...
  return <div className="space-y-4" data-testid="settings-candidate-section-extensions">
    <CandidatePageHeader icon={<Plug size={14} />} title="扩展与工具" description="先选择要管理的能力，再查看详情或连接外部服务。" />
    <div className="flex gap-1 border-b" role="tablist" aria-label="扩展类型" data-testid="settings-candidate-extensions-tabs">
      <button type="button" role="tab" aria-selected={activeTab === 'skills'} onClick={() => setActiveTab('skills')} className="settings-option inline-flex items-center gap-1.5 rounded-t-md px-3 py-2 text-[11px]" data-testid="settings-candidate-extensions-tab-skills" data-selected={activeTab === 'skills' ? 'true' : undefined}><Wrench size={13} />Skills</button>
      <button type="button" role="tab" aria-selected={activeTab === 'mcp'} onClick={() => setActiveTab('mcp')} className="settings-option inline-flex items-center gap-1.5 rounded-t-md px-3 py-2 text-[11px]" data-testid="settings-candidate-extensions-tab-mcp" data-selected={activeTab === 'mcp' ? 'true' : undefined}><Link2 size={13} />外部能力（MCP）</button>
    </div>
    {activeTab === 'skills' && <SettingCard>
      <div className="flex items-start justify-between gap-3"><div><div className="text-[13px] font-medium" style={{ color: 'var(--text-primary)' }}>已安装的 Skills</div><p className="mt-1 text-[10px]" style={{ color: 'var(--text-muted)' }}>Skill 是伙伴完成一类任务时参考的工作方法。</p></div><span className="text-[10px]" style={{ color: 'var(--text-muted)' }}>1 个 · 全局</span></div>
      <div className="mt-4 divide-y rounded-[var(--radius-md)] border" style={{ borderColor: 'var(--border-subtle)' }}><div className="p-3" data-testid="settings-candidate-skill-file-organizer">
        <div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="text-[12px] font-medium" style={{ color: 'var(--text-primary)' }}>文件整理助手</span><span className="rounded-full border px-2 py-0.5 text-[9px]" style={{ borderColor: 'var(--border-subtle)', color: 'var(--text-muted)' }}>内置</span><span className="rounded-full border px-2 py-0.5 text-[9px]" style={{ borderColor: 'var(--border-subtle)', color: 'var(--text-muted)' }}>全局</span></div><p className="mt-1 text-[10px] leading-4" style={{ color: 'var(--text-muted)' }}>帮助你归纳文件、提取重点和整理结果。</p></div><CandidateSwitch checked={skillsEnabled} scope="全局" label="启用文件整理助手" description="" onChange={setSkillsEnabled} testId="settings-candidate-skills-enabled" /></div>
        <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t pt-2" style={{ borderColor: 'var(--border-subtle)' }}><span className="text-[10px]" style={{ color: 'var(--text-muted)' }}>当前状态：{skillsEnabled ? '已启用' : '已停用'}</span><button type="button" onClick={() => setSkillsOpen(!skillsOpen)} className="inline-flex items-center gap-1 rounded-[var(--radius-md)] border px-2.5 py-1.5 text-[10px]" style={{ borderColor: 'var(--border-subtle)', color: 'var(--text-secondary)' }} aria-expanded={skillsOpen} data-testid="settings-candidate-skills-toggle">{skillsOpen ? '收起详情' : '查看详情'}<ChevronRight size={12} className={skillsOpen ? 'rotate-90' : ''} /></button></div>
        {skillsOpen && <div className="mt-3 space-y-3 rounded-[var(--radius-md)] px-3 py-2.5" style={{ background: 'var(--bg-secondary)' }} data-testid="settings-candidate-skills-details"><div className="grid gap-2 text-[10px] sm:grid-cols-3"><div><div style={{ color: 'var(--text-muted)' }}>版本</div><div className="mt-1" style={{ color: 'var(--text-secondary)' }}>v1.2</div></div><div><div style={{ color: 'var(--text-muted)' }}>校验</div><div className="mt-1" style={{ color: 'var(--success)' }}>已校验</div></div><div><div style={{ color: 'var(--text-muted)' }}>可使用工具</div><div className="mt-1" style={{ color: 'var(--text-secondary)' }}>文件读取、文件写入</div></div></div><div className="flex items-center justify-between gap-3 border-t pt-2" style={{ borderColor: 'var(--border-subtle)' }}><span className="text-[10px]" style={{ color: 'var(--text-muted)' }}>仅在当前 Playground 中验证，不写入真实 Skill。</span><button type="button" onClick={() => setSkillsRunState('done')} className="rounded-[var(--radius-md)] border px-2.5 py-1.5 text-[10px]" style={{ borderColor: 'var(--border-subtle)', color: 'var(--accent-fg)' }} data-testid="settings-candidate-skills-run">{skillsRunState === 'done' ? '重新试跑' : '隔离试跑'}</button></div>{skillsRunState === 'done' && <div className="rounded-[var(--radius-sm)] px-2.5 py-2 text-[10px]" role="status" style={{ background: 'var(--accent-subtle)', color: 'var(--accent-fg)' }}>已生成隔离试跑结果</div>}</div>}
      </div></div>
    </SettingCard>}
    {activeTab === 'mcp' && <SettingCard testId="settings-candidate-mcp-card">
