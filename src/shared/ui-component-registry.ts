...
export const UI_COMPONENT_ASSETS = [
  ...
  component({ key: 'behavior.icon-button', labelZh: '图标按钮', labelEn: 'IconButton', descriptionZh: '只使用图标表达的紧凑操作按钮。', category: 'behavior', status: 'playground', implementation: 'custom', sourcePath: 'src/components/playground/UiControlsPanel.tsx', stories: ['默认态', '激活态', '禁用态', '危险操作'], accessibilityNotes: ['必须有可读名称', 'Tooltip 只能作为辅助', '不能只靠颜色表达危险操作'] }),
  ...
  component({ key: 'behavior.select', labelZh: '下拉选择', labelEn: 'Select', descriptionZh: '从稳定枚举中选择单项。', category: 'behavior', status: 'candidate', implementation: 'radix-candidate', sourcePath: 'src/components/playground/FoundationAdvancedStories.tsx', reference: 'Radix Select 候选，尚未引入依赖', stories: ['默认态', '禁用态', '长选项'], accessibilityNotes: ['键盘选择', '标签与控件关联', '当前值可被读屏读取'] }),
  ...
  component({ key: 'behavior.checkbox', labelZh: '复选框', labelEn: 'Checkbox', descriptionZh: '表达可独立开关的布尔选项。', category: 'behavior', status: 'playground', implementation: 'custom', sourcePath: 'src/components/playground/FoundationAdvancedStories.tsx', stories: ['选中', '未选中', '禁用'], accessibilityNotes: ['选中状态可读', '点击文字也能切换', '禁用态不只靠颜色'] }),
  ...
  component({ key: 'developer.markdown', labelZh: 'Markdown 渲染器', labelEn: 'Markdown Renderer', descriptionZh: '正文由 MarkdownRenderer 解析；原始代码由同源 CodeBlock 展示，供基础故事、候选和正式审阅共享。', category: 'developer', status: 'adopted', implementation: 'custom', sourcePath: 'src/components/MarkdownRenderer.tsx', stories: ['正文', '代码块', '原始代码与围栏', '长文 aside'], accessibilityNotes: ['标题层级连续', '代码块复制保留原文且操作槽固定', '原始代码不解释为链接或 Mermaid', '外部链接经过安全处理'] }),
  component({ key: 'developer.diff-viewer', labelZh: '差异查看器', labelEn: 'Diff Viewer', descriptionZh: '查看 Prompt、Skill 或文件版本变化。', category: 'developer', status: 'candidate', implementation: 'reference-only', sourcePath: 'src/components/playground/FoundationAdvancedStories.tsx', reference: 'WISH-014 Skill Diff 的共享候选', stories: ['并排差异', '行内差异', '长文件'], accessibilityNotes: ['新增删除不能只靠颜色', '键盘可定位变更', '保留原始文本复制'] }),
  ...
  component({ key: 'layout.workspace-browser-shell', layer: 'experience', labelZh: '浏览器工作区壳', labelEn: 'Workspace Browser Shell', descriptionZh: '正式右坞中的浏览器入口与后端未接入空态。', category: 'layout', status: 'adopted', implementation: 'custom', sourcePath: 'src/components/chat/right-dock/BrowserPanel.tsx', stories: ['未接入空态', '地址栏占位'], accessibilityNotes: ['未接入状态明确可读', '不暗示外站已可访问'] }),
  component({ key: 'layout.workspace-sidechat-shell', layer: 'experience', labelZh: '侧边聊天工作区壳', labelEn: 'Workspace Side Chat Shell', descriptionZh: '正式右坞中的侧边聊天入口与后端未接入空态。', category: 'layout', status: 'adopted', implementation: 'custom', sourcePath: 'src/components/chat/right-dock/SideChatPanel.tsx', stories: ['未接入空态', '会话契约提示'], accessibilityNotes: ['未接入状态明确可读', '不伪造消息或生成状态'] }),
