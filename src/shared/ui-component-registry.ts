...
export const UI_COMPONENT_ASSETS = [
  ...
  component({ key: 'layout.settings-layout', labelZh: '设置布局', labelEn: 'Settings Layout', descriptionZh: 'Playground 与正式设置共用的分组导航、移动端标签和内容滚动容器。', category: 'layout', status: 'adopted', implementation: 'custom', sourcePath: 'src/components/settings/SettingsLayout.tsx', stories: ['桌面导航', '窄宽标签', '长内容滚动'], accessibilityNotes: ['当前页有 aria-current', '移动端标签可键盘操作', 'active/hover 不改变布局'] }),
  component({ key: 'behavior.setting-card', labelZh: '设置卡片', labelEn: 'Setting Card', descriptionZh: '设置页面承载独立字段组的统一容器。', category: 'behavior', status: 'adopted', implementation: 'custom', sourcePath: 'src/components/settings/SettingsFields.tsx', stories: ['字段分组', '范围徽标', '长内容'], accessibilityNotes: ['字段层级清晰', '不依赖左侧色条', '内容换行不挤出容器'] }),
