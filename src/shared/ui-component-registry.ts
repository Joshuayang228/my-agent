...
export const UI_COMPONENT_ASSETS = [
  ...
  component({ key: 'layout.character-shelf', layer: 'experience', labelZh: '角色架', labelEn: 'Character Shelf', descriptionZh: '正式与候选共享角色卡片及当前主角标记，切角由调用方管理。', category: 'layout', status: 'adopted', implementation: 'custom', sourcePath: 'src/components/companion/CharacterShelfContent.tsx', stories: ['主角切换', '长文', '失败恢复'], accessibilityNotes: ['卡片按钮具有选中语义', '当前标记固定占位', '长名称换行', '在途禁止重复切角'] }),
  ...
  component({ key: 'layout.chat-composer', layer: 'experience', labelZh: '对话输入区', labelEn: 'Chat Composer', descriptionZh: '正式与候选共享 Foundation 输入、附件和发送停止操作，业务回调由调用方提供。', category: 'layout', status: 'adopted', implementation: 'custom', sourcePath: 'src/components/chat/ChatComposer.tsx', stories: ['短长输入', '流式停止', '窄屏长模型名', '候选隔离'], accessibilityNotes: ['输入有可读标签', '发送与停止尺寸一致', '输入高度有界', '按钮有名称'] }),
  component({ key: 'behavior.chat-approval', layer: 'experience', labelZh: '对话审批入口', labelEn: 'Chat Approval', descriptionZh: '共享基础按钮与审批菜单，保存回调由正式入口提供，候选禁用。', category: 'behavior', status: 'adopted', implementation: 'custom', sourcePath: 'src/components/chat/ChatApprovalControl.tsx', stories: ['三种审批方式', '保存失败与在途', '键盘与固定尺寸'], accessibilityNotes: ['菜单单选语义', '键盘导航和焦点回收', '保存成功后才更新模式', '错误不暴露内部信息'] }),
  component({ key: 'layout.chat-message-frame', layer: 'experience', labelZh: '对话消息外框', labelEn: 'Chat Message Frame', descriptionZh: '正式与候选共享消息排列、头像、身份和气泡，真实正文与操作由调用方注入。', category: 'layout', status: 'adopted', implementation: 'custom', sourcePath: 'src/components/chat/ChatMessageFrame.tsx', stories: ['用户与助手', '长文本与代码', '历史和召唤身份'], accessibilityNotes: ['时间使用消息记录', '未知角色不猜名称', '头像尺寸固定', '长文不撑宽消息列'] }),
  // 行为组件：先登记候选，不因为进入目录就假装已安装 Radix。
  // 基础控件故事：当前以正式样式和隔离 fixture 建场，尚未回流为独立生产组件文件。
  component({ key: 'behavior.action-button', labelZh: '文字动作按钮', labelEn: 'ActionButton', descriptionZh: '用于恢复、打开、确认和低强调导航等带文字的紧凑动作；固定操作槽并复用语义色。', category: 'behavior', status: 'adopted', implementation: 'custom', sourcePath: 'src/components/foundation/ActionButton.tsx', stories: ['强调动作', '中性动作', '低强调导航动作', '禁用态', '窄宽与固定高度'], accessibilityNotes: ['按钮名称直接可见', 'hover/focus/disabled 不改变尺寸', '危险色必须由调用方明确选择'] }),
  component({ key: 'behavior.button', labelZh: '按钮', labelEn: 'Button', descriptionZh: '触发提交、确认、导航和恢复动作的通用文字按钮；业务层注入文案和回调。', category: 'behavior', status: 'adopted', implementation: 'custom', sourcePath: 'src/components/foundation/Button.tsx', stories: ['主要操作', '次要操作', '生成动作', '禁用态', '加载态'], accessibilityNotes: ['按钮名称清楚', '禁用态不能只靠颜色', '处理中保留原文宽度和操作槽', 'hover/focus/disabled 不改变外部几何'] }),
  component({ key: 'behavior.input', labelZh: '输入框', labelEn: 'Input', descriptionZh: '承载短文本、长文本和搜索输入的基础控件；正式工作区与 Foundation 故事共用 TextField。', category: 'behavior', status: 'adopted', implementation: 'custom', sourcePath: 'src/components/foundation/TextField.tsx', stories: ['空值', '有值', '长文', '低强调焦点', '禁用态', '工作区地址/命令/侧聊'], accessibilityNotes: ['标签与输入关联', '长文不挤出容器', 'IME 组合输入不误提交', 'hover/focus/disabled 不改变外部几何'] }),
  ...
  component({ key: 'state.empty', labelZh: '空状态', labelEn: 'Empty State', descriptionZh: '没有数据或尚未开始时的引导；业务层注入真实的下一步动作。', category: 'state', status: 'adopted', implementation: 'custom', sourcePath: 'src/components/foundation/EmptyState.tsx', stories: ['默认态', '窄宽长文', '浏览器未加载', '角色架无数据'], accessibilityNotes: ['说明为什么为空', '操作入口不应伪造能力', '文本层级清楚', '有无操作时保留固定操作槽'] }),
  component({ key: 'state.error', labelZh: '错误状态', labelEn: 'Error State', descriptionZh: '请求、权限和配置失败时的恢复入口；错误原因与真实回调由调用方提供。', category: 'state', status: 'adopted', implementation: 'custom', sourcePath: 'src/components/foundation/ErrorState.tsx', stories: ['未配置', '权限拒绝', '上游失败', '浏览器加载失败', '角色架读取失败'], accessibilityNotes: ['提供可执行恢复动作', '不暴露内部堆栈', '错误语义可读', '有无操作时保留固定恢复槽'] }),
  ...
  // 开发工具。
  component({ key: 'developer.tool-callback', labelZh: '工具调用卡', labelEn: 'Tool Callback', descriptionZh: '显示工具运行、结果和失败状态。', category: 'developer', status: 'adopted', implementation: 'custom', sourcePath: 'src/components/chat/callbacks/ToolCallbackList.tsx', stories: ['运行中', '完成折叠', '失败展开', '生成图片与读取失败'], accessibilityNotes: ['状态变化可读', '折叠按钮有名称', '长结果可以滚动', '图片读取与失败态保留尺寸，查看原图操作有名称'] }),
  ...
  component({ key: 'layout.mcp-service-list', layer: 'experience', labelZh: 'MCP 服务清单', labelEn: 'MCP Service List', descriptionZh: '正式与候选共用数量、添加入口和空态，连接和保存状态由调用方注入。', category: 'layout', status: 'adopted', implementation: 'custom', sourcePath: 'src/components/settings/McpServiceList.tsx', stories: ['空清单', '多服务', '添加期间禁用'], accessibilityNotes: ['添加按钮使用 Foundation', 'hover 与禁用不改变尺寸', '候选不读取生产数据'] }),
