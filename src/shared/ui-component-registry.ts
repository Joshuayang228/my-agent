...
export const UI_COMPONENT_ASSETS = [
  ...
  component({ key: 'developer.markdown', labelZh: 'Markdown 渲染器', labelEn: 'Markdown Renderer', descriptionZh: '正文由 MarkdownRenderer 解析；CodeBlock 与 Mermaid 从所在区域读取语义主题，供基础故事、候选和正式工作区共享。', category: 'developer', status: 'adopted', implementation: 'custom', sourcePath: 'src/components/MarkdownRenderer.tsx', stories: ['正文', '代码块', '原始代码与围栏', '长文 aside', '局部四主题与图表'], accessibilityNotes: ['标题层级连续', '代码块复制保留原文且操作槽固定', '原始代码不解释为链接或 Mermaid', '图表失败显示提示与原文且可恢复', '外部链接经过安全处理'] }),
  ...
  component({ key: 'layout.world-details-panel', layer: 'experience', labelZh: '人物世界生活面详情', labelEn: 'World Details Panel', descriptionZh: '读取角色资产与生活动态；文化角、家居和足迹通过 WorldLivingContent 与 Playground 共享展示，不复制生活样张。', category: 'companion', status: 'adopted', implementation: 'custom', sourcePath: 'src/components/WorldDetailsPanel.tsx', stories: ['文化角', '家居', '足迹', '加载与错误重试'], accessibilityNotes: ['内容来自真实 companion IPC', '只读页面不提供虚假编辑', '刷新失败保留内容且可重试', '不同主角响应不混合', '固定尺寸刷新操作槽', '摘要和关联笔记同时保留'] }),
  ...
  component({ key: 'layout.mcp-connection-form', layer: 'experience', labelZh: 'MCP 连接添加表单', labelEn: 'MCP Connection Form', descriptionZh: '正式设置与 Playground 共用的测试后保存连接流程；组件不持有生产连接或凭据存储。', category: 'layout', status: 'adopted', implementation: 'custom', sourcePath: 'src/components/settings/McpConnectionForm.tsx', stories: ['远程 Streamable HTTP', '本地 stdio', '测试与工具选择', '取消与保存失败'], accessibilityNotes: ['测试成功前不能保存', '取消和关闭会清理测试连接', '长工具清单内部滚动', '测试、保存操作槽固定'] }),
