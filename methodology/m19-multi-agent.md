...
**我们的实现（G4，2026-07-05 补做）**：`delegate_task` 从 toolContext 读父 Agent 的 executionMode，传给子 Agent。`resolveChildExecutionMode` 实现"只降不升"——定义模式严格度序 `auto(0) < confirm-all(1) < plan-first(2)`，子 Agent 默认想用 auto（省确认），但若父级更严则必须继承父级严格度，**不能逃逸到更宽松**。这是纯函数，单测覆盖四种父模式。

**判据**：权限传递的方向是"子不能比父松"。实现上不是简单复制父模式，而是 `min(子期望的宽松度, 父允许的宽松度)`——子 Agent 可以更保守，但不能更激进。
...
**我们的实现（2026-07-05 补做 continue）**：现在 spawn 和 continue 都有了。
- **spawn**：`delegate_task` 每次新建子 Agent，跑完把实例（含 messages 历史 + 工具集 + 执行模式）存进 `subagent-registry`，返回 agentId。
- **continue**：新增 `continue_task(agent_id, message)` 工具（对应 CC SendMessage），从注册表取实例、追加消息、复用其上下文和工具集续跑。

**我们和 CC 的一个关键差异**：CC 是异步多 worker 总线（coordinator 发消息给独立跑的 worker，结果异步回来）；我们是**单一主对话流**，子 Agent 在父 Agent 的一次工具调用里同步跑完。所以我们的 continue 是"同步续跑"——`continue_task` 工具调用里把子 Agent 的 loop 再跑一轮，不是给一个后台存活的 worker 发消息。实例生命周期绑定当前会话，会话结束 `clearSessionSubAgents` 清理。

**判据**：continue 的本质是"复用上下文"，不在于同步还是异步。单对话流产品用同步 continue 就够——保留子 Agent 的 messages 历史，追加消息续跑，就实现了 CC continue 的核心价值（研究者带着研究上下文直接做实现）。异步总线是 CC 多 worker 场景的需要，不是 continue 的本质。
...
回看时发现 M8 首轮的"暂缓项"里混着**真实 bug/安全漏洞**（不是"未来增强"），一次补齐：

- **G4 权限只降不升**（安全）：`resolveChildExecutionMode` 实现只降不升，子 Agent 不能从 confirm-all/plan-first 父级逃逸到 auto。见 §六。
- **G5 子 Agent 传 toolContext**（正确性 bug）：`runSubAgent` 调 agentLoop 原来没传 toolContext——子 Agent 里的所有工具（file_read/shell_exec 等）拿不到 workdir/sessionId/signal，一用文件工具就 workdir 错乱。这是个**隐藏了整个 M8 首轮的 bug**：delegate_task 破损（P0）修好后没人真跑过子 Agent 里的文件工具，所以没暴露。
- **G6 role 角色系统**：`AGENT_ROLES` 预设 researcher/coder/analyst，各带默认工具集 + 只读性，可被显式参数覆盖，自由字符串回退。见 §三/§五。
...
- **Swarm 模式**：去中心化 SQLite 任务队列 + 依赖 DAG。Alice Ch.6 第三种模式，适合跨会话持久化、任务动态产生的场景。独立大工程，产品需求不明确时不引入，认知框架已在 §三 写全。

## 沉淀：多 Agent 的设计检查清单

1. 这个任务是"信息积累型"还是"并发执行型"？前者用单 Agent，后者才考虑多 Agent。
2. 子 Agent 的上下文真的隔离了吗？中间过程有没有污染父 Agent？
3. 子 Agent 的工具集是受限的吗？有没有给它不该有的工具（delegate_task / continue_task / remember）？
...
8. **子 Agent 的工具拿到 toolContext 了吗？**（workdir/sessionId/signal）——否则文件类工具会错乱。
