...
1. **无失败重试**：任务失败后直接标记 failed，不会自动重试（指数退避逻辑未实现）。
2. **无 token 分离**：后台任务的 LLM 调用 token 混入主会话统计（`caller` 字段已埋点，但 token 归因未分离）。
3. **无 UI 可见状态**：前端收到事件后只打 console，没有任务列表 UI。
4. **无长任务断点续接**：任务必须一次执行完，中途崩溃会从头重做（幂等性由业务函数自行保证）。
5. **无 linked span**：后台任务没有独立 span，影响 trace 耗时准确性（M7 依赖）。

---

## 八、下一步扩展方向

按 `m11-task-lifecycle.md` § 九 优先级判断：

1. **前后台 token 分离**（影响 token 预算准确性）
2. **完成通知 UI**（侧边栏或底部状态 pill）
3. **失败重试 + 指数退避**
4. **断线重连后任务状态同步**
5. **长任务断点续接**（等真实需求出现时再设计）

v2 已完成基础持久化和恢复机制，后续扩展可以在此基础上增量添加。
