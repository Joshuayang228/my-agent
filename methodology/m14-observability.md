...
> **一个前置边界**：可观测性（记录发生了什么）和日志（记录诊断信息）是两个不同的问题，解法也不同。前者靠结构化 Span，后者靠文本日志。M7 主体做 Trace（Span）；日志文件落盘作为独立子问题（G4）已于 2026-07-08 补做（见 §十）。
...
**我们的实现**：主要做 Traces（SpanType 分类 + 父子嵌套），轻度覆盖 Metrics（getCallerStats 按 caller 聚合耗时和 token）。Logs 有 createLogger，2026-07-05 补做了文件落盘（G4，见下）。
...
**G4 日志文件落盘**（✅ 2026-07-08 补做）：
- 原现状：只有 console 输出，关窗口日志消失
- 实现：`logger.ts` 在 `log()` 内部加落盘层，与 console 并行——`app.getPath('logs')/agent-YYYY-MM-DD.log` 按日期分文件，同步 append 保证顺序，惰性初始化（首次写才开文件）；启动时 `cleanupOldLogs` 删超过保留期（默认 7 天）的旧日志
- **关键设计：降级不崩**——`app.getPath` 在非 Electron 环境（vitest / 纯 Node）不可用时，落盘整体跳过，console 输出照常。这让 44 个 logger 调用方和所有测试零改动
- 纯逻辑（哪些文件算过期）抽成导出函数 `selectExpiredLogs(files, now, keepDays)` 便于单测，副作用（读目录、删文件）留在 `cleanupOldLogs` 里——延续 M5 坑 2「难测＝逻辑和副作用耦合」的分离原则
- **脱敏另算**：日志写盘目前不过滤 API key / token。§八 隐私原则要求的「记元数据不记内容」在落盘层尚未强制，作为独立小任务后续做（写盘层加一道字段过滤）
...
- ~~G4 日志文件落盘~~ ✅ 2026-07-08 补做（文件落盘 + 按日期轮转 + 保留最近 7 天；脱敏另算，暂缓）
- G4 日志脱敏（写盘时过滤 API key / token）——暂缓，待独立做
