...
关键逻辑的注释必须同时包含以下三项，缺一不可：

- **背景**：这段代码要解决的真实问题，以及在哪个业务场景下存在。
- **设计意图**：为什么选择当前做法，以及放弃了哪个更直觉的做法。
- **关键约束**：哪些条件、顺序、值或外部契约不能改变；改变后会踩什么坑。

禁止写功能复述式注释（例如“设置变量为 true”“循环数组”）；代码本身能直接表达的内容不用注释重复。详规见 `agent-skills/typescript-guidelines.md`。

#### 规则二：代码保护与多 Agent 授权

- **禁止静默删除**：删除任何现有函数、分支、配置项、依赖或其他行为前，必须在任务记录中说明删除对象、原因和影响范围。
- 删除授权可以来自：用户对当前任务的明确指令、已确认施工合同中的明确范围，或主 Agent 对子 Agent 的明确文件 / 行为范围分配；仅仅“看起来没用”不构成授权。
- **越界删除必须停下**：如果删除对象不在已批准范围内，或删除会影响其他 Agent 正在使用的契约、入口、测试或数据，应先通知协调者 / 用户并等待确认。
- **看不懂的代码不许删**：遇到没有注释且用途不明的代码，默认它可能在解决未知线上问题；先定位调用方、测试和历史背景，不得猜测后删除。
- **重构不顺手做**：修 bug 只修 bug；看不惯的旧代码另开任务，不在当前任务中顺手重构。

#### 多 Agent 协作规则

- 每个 Agent 开始前必须声明：任务目标、允许修改的文件 / 目录、明确不碰的文件，以及交付验证命令。
- 文件和接口契约默认按 Agent 认领范围隔离；未认领文件只能读取，不能顺手修改。共享类型、注册表、测试和文档属于高冲突文件，修改前必须检查是否已有其他 Agent 改动。
- 多 Agent 施工期间，工作区允许短暂出现编译失败、测试失败或注册表不一致；这只是过程态，不得提交、推送或对外宣称完成。协调者必须在交付前合并完整契约，并按顺序通过类型检查、测试、构建和文档门禁。
- 发现其他 Agent 的改动与当前任务冲突时，先记录冲突文件、实际差异和受影响契约；不得 reset、覆盖、改测试掩盖失败，或擅自选择删除 / 保留其中一方。
- 每个可交付批次必须能独立说明完整边界、明确不做项、文件归属、验证结果和未解决风险；过程中的半成品可以留在工作区，但不能作为交付物。
#### 规则三：不接受分期交付

- 不许“先做简版，后续再加”；做不完就缩小范围，不降低已交付部分的完成度。
- 禁止 TODO 占位交付、返回假数据的空函数、只有界面没有逻辑的按钮，以及“大体能跑”的未验证流程。
- 交付前必须走一遍完整流程，包括失败路径。
- 如果范围确实过大，必须在动手前说明并共同缩小范围；明确不做的内容写进施工合同或对应文档。
- 缩小范围时必须明确：本次完整边界是什么、哪些明确不做、剩余部分是否会让已交付内容变成半成品。

### 调试铁律：禁止猜测性修复，先 Log 再改码

遇到报错，不许直接改代码试试看。修复之前必须先回答：

1. **出错的确切位置在哪**：哪个文件、哪一行、哪次调用。
2. **那一刻的实际数据是什么**：变量的真实值，而不是推测值。
3. **它和预期的差在哪**：预期行为、实际行为，以及差异如何导致故障。

三问答不上来时，必须先复现并增加临时日志，记录足以区分假设的关键变量，跑一遍取得真实数据后再改码。一次只改一个地方，改完立即验证，不得一口气修改多处再统一测试。

每次修复都必须声明影响范围和重点回归路径。临时日志在修复确认后必须删除，或转为不泄露敏感信息的正式结构化日志。禁止用“加个 try/catch 先让它不报错”代替定位和修复；异常必须被处理、转换、上抛或明确记录，不能静默掩盖。
...
功能开发完成且测试通过后**必须**立即 commit + push；多 Agent 过程态除外，过程态必须在交付前收口，不得以过程态提交或推送：
...
- **Phase P0 · Playground 候选**：任何 UI 视觉、布局、IA、入口位置、交互状态或文案层级调整，必须先只落在 Playground 故事格 / 页面组合、对应施工合同和隔离测试中。除非用户当轮明确要求“直接修正式页面”，否则禁止先改 `src/App.tsx`、正式 Sidebar、Settings、人物世界等生产 UI 行为。
- **用户确认是回流许可证**：必须让用户在 Playground 实际查看截图或可交互候选态，并收到明确“可以回流 / 就这样改正式页面”等许可；“方向合理”“继续研究”“先做做看”不等于生产回流授权。
- **候选态不得偷改默认行为**：P0 优先 import 正式组件并通过静态 props、隔离 wrapper、fixture 或 Playground-only class 表达差异；不得为了候选态修改生产组件默认 props、默认布局或正式入口位置。
- **Phase P1 · 正式回流**：取得明确许可后，正式页面与故事格同轮更新，并补深浅主题、窄宽、溢出和关键交互验收。若生产实现发现候选方案不可行，必须返回 Playground 重新展示，不得静默换方案。
- **越闸处理**：发现未确认就修改了生产 UI，立即停止继续扩改，记录到 `docs/rules-feedback.md`，先恢复“Playground 候选 → 用户确认 → 正式回流”的链路。
