...
> 状态：暂缓（产品决策，2026-09-14）
> 生命周期：已完成施工快照（冻结）；当前阶段不安装原生依赖、不修改正式终端。完整 PTY 只有在产品出现明确交互式终端需求后重新确认并开工。
> 来源：`WISH-019`、`docs/requirements/chat-right-dock.md`、`docs/requirements/playground-workspace-five-tools-v1.md`

## 1. 背景与目标

正式 ChatRightDock 当前已经回流了终端界面，但实现是 `child_process` 命令控制台：可以输入一次命令并查看输出，不能支持 `vim`、REPL、全屏程序、光标控制或终端尺寸变化。经过对 Alice 快照的产品与实现审计，当前产品目标优先是让 Agent 的命令执行可见、可控、可恢复，而不是提供独立 IDE 终端；因此本合同暂缓，不把交互式终端能力当作当前回流完成条件。
...
只有出现明确的产品需求（例如用户需要在 My Agent 内持续使用 shell、REPL、`vim` 或其他交互式 CLI），并重新确认本合同后，才进入以下确认闸。
