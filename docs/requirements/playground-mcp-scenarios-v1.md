...
列表右上角使用带边框的「添加连接」按钮，打开独立表单；顶部追加远程填写、本地填写、校验失败、添加连接中、获取工具成功样张，直接切换。远程填写名称、HTTP(S) URL、认证方式与可选令牌；本地填写名称、启动命令、逐行参数和环境变量。校验无效输入，测试后选择工具、保存到本地列表；取消不改变已有服务，修改连接字段使测试结果失效。连接中可取消，示例结果与真实网络分离。不启动进程、不请求网络、不保存凭据；JSON 导入与真实 OAuth 仍不在本轮范围。
...
本轮完整交付的是隔离场景审阅，不是可连接外部服务的管理器。直达的连接中场景保持展示直到取消或切换场景；表单中主动测试会短暂展示连接中，再产生标注为样张的工具结果，取消与卸载清除计时器。JSON 导入和真实 OAuth 流程不在本轮范围。
