...
R01 消息外框回流边界（2026-09-20）：ChatMessageFrame 统一用户 / 助手头像、排列、身份行与用户气泡，App 保留原有 Markdown、推理、工具回合、记忆引用、编辑及操作处理器，候选仅传样张 children / 时间标签。正式名称按会话 roleId 读取既有主角与通讯录映射，当前主角用当前名称，未知角色回退「伙伴」；时间来自 msg.timestamp，不复制「刚刚」。允许修改共享外框、App、ChatSurface、组件 / 体验注册表、共享 Unit、正式 UI 验收与模块 / 质量 / 账本；删除范围仅为重复消息布局及其图标导入，消息图片最大宽度服从气泡。无 IPC、数据结构、模型或记忆检索变更；审批菜单仍单独核验，全产品状态不提前采用。必测历史 / 召唤身份、真实时间、四主题宽窄长文与代码、hover 几何、编辑取消 / 删除与现有工具卡回归。
...
| 范围 | 当前已有实现 / 证据 | 剩余归属 |
|---|---|---|
| R01 对话与导航 | App / PrimarySidebar；欢迎、输入、审批、消息外框均与候选共享，真实业务回调保留；对应定向 UI 已通过 | S4、S6：正式组合态和剩余生命周期 |
...
| R05 记忆 | 四类管理、长文编辑、敏感确认及真实 memory IPC；onboarding 有完整重启 CRUD，S1 索引恢复已有证据 | S6：当前正式组合验收；S2 独立嵌入与 RAG 后续暂缓 |
| R06 模型与生图 | 共享连接 / 用途 / 高级表单、配置工厂、真实协议、生图与窗口诊断取消已实现 | S2、S4；付费外部服务结果不在本地协议证据内 |
| R07 数据与隐私 | 共享数据页；核心事务、媒体崩溃恢复、朋友圈历史与赞评备份已有独立 Electron 测试，S1 派生索引恢复已验证 | S6：当前最终组合验收，不重开 S1 |
...
| R12 人物世界 | 六面入口及 IPC 已接通，WorldHub / WorldProfileHeader 与候选同源；硬编码衣柜 / 文化 starter 已移除 | S3、S6：旧开发库来源和六面当前组合分别核验 |
...
R12 六面分别验收：朋友圈核对赞评及备份链；衣柜、文化角核对无硬编码默认资产后的真实空态与用户读写；家居核对 Role Pack 空间 / 物件来源；通讯录核对 roster、忙闲与开聊；足迹核对事件地点和用户地点。已移除衣柜 / 文化 starter 不再作为缺陷反复施工；六面角色隔离、当前展示和真实持久化仍归 S6，不因一个“可见”断言整体采用。
...
| S3 人物生活数据来源 | 已收口：衣柜 / 书架 / 文化硬编码 starter 已移除；住所 / 地点只从 `world.default.json` 读取；无来源角色保持真实空态；来源矩阵由 `companion-source-boundary.test.ts` 固化 | 每项正式初始内容来自已确认 Role Pack / 用户操作 / 已发布事件；无来源用真实空态；候选样张保持隔离。不把旧开发库记录当成人物事实，不编造人物设定 |
| S4 草稿生命周期边界 | 已收口：普通设置队列、权限在途请求、模型草稿和 MCP 向导都接入 `SettingsPanel.prepareToLeave` 与 `beforeunload`；正式 Electron 已覆盖隐藏再显示、内部导航、显式退出和重载，结构门禁由 `settings-lifecycle-boundary.test.ts` 固化 | 不新增多窗口产品或自动持久化未保存凭据；OAuth 后续除外。后续只处理新功能引入的生命周期回归，不重复修已关闭故障 |
| S5 验收可靠性 | watcher 已排除文档并保留源码刷新，含交叠事件单测；外部模型脚本已使用新连接表单和逐例独立目录。onboarding 22 项全通过，模型路由、人物世界、工作区、终端、权限、记忆等代表入口已用独立 Electron 进程复跑；`beforeAll` 仅共享本地服务与测试进程资源，不再作为产品数据依赖 | 主线证据已收口；资源级 worker 重建仍可单独优化，但不阻塞 S6。其他已解决项保留证据，不重复施工；偶发失败须有可解释证据，不靠放宽断言或反复重跑宣称修复 |
| S6 全产品正式验收与采用 | 四个 `experience.*` 已登记为 `adopted`；RAG 与 MCP OAuth 按用户决定暂缓，不纳入本批完成 | 对照 R01–R14 与 R12 六面，从正式 App 逐项操作并检查深浅 / 宽窄 / 长文 / hover / 失败态；核实 Foundation 实际复用与夹具隔离；跑最终回归，更新采用状态、模块卡和账本，稳定事实吸收后冻结合同并提交推送 |
