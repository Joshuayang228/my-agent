...
| 保存 / 离页 / 发现迟到 | 整组保存失败恢复与请求结果归属隔离已接通；独立草稿离页仍未保护 | SQLite 故障 / 重开与正式入口 Electron；受控请求逆序、换 Key、取消 / 保存失败及离页结果隔离 UI；继续补草稿离页 |
