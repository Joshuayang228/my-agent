...
| 编号 | 当前缺口与证据 | 完成条件 |
|---|---|---|
| S1 记忆派生索引恢复 | memory-store 的 afterCommit 只向进程内队列发布向量任务；data-export 成功提交后调用，退出 / Embedding 失败后无持久恢复证据 | SQLite 仍为事实源；导入后立即退出及向量失败，重启 / 重试能补齐且不重复、不串角色；真实检索验证，不只检查任务记录 |
| S2 模型与嵌入配置收口 | memory-store 仍以 config.apiKey 非空为同步条件；embeddings.ts 的 embeddingUnavailable 为进程全局；旧模型键消费者尚未完整收敛 | 支持已声明的本地无 Key 嵌入；失败按连接隔离且切换可恢复；旧键清理沿调用链证明不破坏独立嵌入；配置、手动记忆与检索走同一真实链 |
