...
## 2.4 主题候选更新（2026-08-29）

- 移除与宣纸过于接近的铜版；主题候选不再沿用多个暖白 / 棕金方向。
- 新增「瓷青」：冷白底、青瓷层、靛蓝交互色，作为明确的冷色浅色方向。
- 新增「绛紫」：深莓底、烟紫面、玫瑰色交互色，作为区别于曜石的深色方向。
- 主题比较仍只改变 Playground 内的隔离样张，不写入生产主题注册表。
...
3. 主题对照先提供 4 个明显区分的候选方向：瓷青、曜石、松烟、绛紫。候选仅是 Playground 隔离 fixture，不改变 `DESIGN_THEME_ASSETS` 和正式主题。
