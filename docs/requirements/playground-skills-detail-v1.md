...
3. 本轮不从列表暴露详情入口；版本、作者、来源、文件树和使用范围只保留为后续独立详情设计的候选。
4. 覆盖单个列表、多个列表和详情审阅等隔离状态。

## 3. 技术方案（How）

- 仅修改 `SettingsExperienceCandidate.tsx` 中的 Renderer fixture；不调用 `window.electronAPI`。
- `CapabilityPage` 在 `mode === 'skills'` 时只维护启用状态；列表行不承担详情导航。
- 列表使用扁平行结构；名称只承担识别，开关是唯一直接管理动作；详情通过 Playground 状态样张单独审阅，不从默认列表增加隐式入口。
