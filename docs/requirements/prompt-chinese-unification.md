...
- Prompt 先用简体中文写清楚意图、行为、优先级、边界与例外。
- 英文只保留在必要的技术术语、工具名、JSON key、代码标识、协议 token、外部原文或 canonical name 中。
- 同一条行为规则只保留一份自然语言表达，禁止中文规则后再复制一份英文规则。
...
参考 Alice 的实现思路：稳定主人设模板保留动态插槽，角色与用途通过 Prompt 映射组织，`<alice_life_context>`、`<alice_world_facts>` 等动态上下文独立注入；我们借鉴其分层方式，不复制其产品语义或具体文案。
...
- 中英逐句混写会让模型面对重复规则和不清晰优先级，因此“中文先写、英文只保留必要术语”必须作为评审门槛。
