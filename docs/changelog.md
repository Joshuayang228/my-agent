...
- Playground 基础组件工作台新增 Markdown 渲染、文件树、资产目录、标签切换、提示条、加载指示器和分栏拖拽等基础故事。
- 基础资产总览现在同时展示已建立故事和候选能力，避免注册表资产在 Playground 中“有登记但看不见”。
- 候选组件继续明确标记为候选，不伪造安装状态；本轮只扩充 Playground 验收面，不改变正式产品默认页面。
