...
## 2026-09-07 · 连接卡片操作与模型下拉候选

- 按用户两条浏览器注释，将获取已有模型 / 测试连接移到每个连接卡片右上角，手动添加改为加号；折叠按钮与操作按钮是独立目标。
- 复核 CC-Switch 的 ModelInputWithFetch / ModelDropdown，采用下拉选填后再确认的流程，移除铺开的获取结果按钮。已添加项禁用，手填 ID 和 Enter 提交保留；草稿 / 候选按连接隔离，重置样张清理未完成的获取。
