...
- 下一步：Foundation 全量复用核验，优先处理按钮、输入、菜单等仍为局部 JSX 的控件，并继续主进程类型门禁。Unix 终端仍需实机验证，完整 PTY 仍见 WISH-019，浏览器当前不是支持登录/脚本的完整浏览器。目标未完成。
- 门禁发现：默认 `npx tsc --noEmit` 只检查 src，单独主进程检查存在既有跨项目配置和类型错误；不得再以默认 tsc 通过声称主进程全量类型通过。见 WISH-043。

### 既有施工记录

- 2026-09-14：正式工作区地址栏、命令控制台、侧边聊天和 Foundation 输入故事统一接入 `TextField`；补充事件类型与目录测试，未改变提交、停止、流式或 IPC 行为。全量基础控件复用仍在进行。
- 2026-09-14：正式浏览器、终端和侧边聊天的图标操作槽统一接入 `IconButton`；补充真实调用门禁与工作区 E2E，未改变 IPC、流式或进程生命周期行为。全量基础控件复用仍在进行。
