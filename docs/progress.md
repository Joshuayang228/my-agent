...
- 工作区候选改为右侧工作坞定位；默认态不打开，文件任务按需出现，审阅只展示差异上下文，确认与恢复动作留在 Chat。
- 正式 Settings、Chat、MCP / Skill IPC、任务队列与文件系统均未改变。
- 扩展页继续收平为直接命名的 Skills / MCP 两个 Tab；Skill 条目去掉“当前状态”，把更多信息降到辅助详情层，避免卡片套卡片。
- Skills Playground 已收平为列表行：只保留名称与纯开关，详情、文件树和内容预览不从当前列表暴露；全部仍为 Renderer fixture。
- Skills Playground 增加“单个 / 多个 / 详情”三种独立审阅状态；Skills 详情状态已调整为标题同行开关；多个状态改为独立卡片，触发条件按 description 语义展示。；默认仍是名称 + 纯开关，详情状态用于审阅元信息和文件预览。

## 2026-09-08 · 记忆管理用户故事候选

- 按用户确认的第一性原理，将设置里的“记忆”从跳转空壳改为长期信息管理入口：第一屏提供已记住数量、最近更新、搜索和添加记忆。
- 复用已有四类记忆边界：身份信息、协作习惯、沟通偏好、我们之间；添加记忆支持分类、搜索、编辑和删除，来源仍只在 Debug 样张开启后显示。
