...
S6 / Foundation 输入基线正式回流（2026-09-23）：Playground 验收通过后，将无内嵌边框与低对比焦点反馈回流到正式记忆编辑、朋友圈评论和工作区输入；真实保存、评论、终端、浏览器和隐藏态几何保持不变。全量 Unit 185/185、正式 UI E2E 27/27、根 `tsc`、build、assets 与 docs 门禁通过。

S6 / 朋友圈卡片高度收口（2026-09-23）：朋友圈动态卡片明确按内容高度收缩并从列表顶部对齐，修复短动态底部出现大块空白的问题；正式朋友圈赞评与卡片高度回归通过。

S6 / 人物世界衣柜重复壳审计（2026-09-23）：移除衣柜 / 书架面板重复的“物什”标题、主角说明、关闭和常驻刷新栏；人物世界外层负责页面上下文，面板保留衣柜 / 书架子切换，读取异常时提供面内重新读取。正式衣柜读取隔离与恢复在两主题、两宽度下 12/12 通过。

S6 / TextField 默认边界收口（2026-09-23）：Foundation `TextField` 默认声明 `border-0`，由组合控件自行决定是否提供边界；显式使用 `border` 的表单保持原有样式，搜索、工具栏等外层容器不再露出浏览器原生双边框。记忆搜索四主题桌面 / 窄屏用例 8/8、Foundation 定向 Unit 36/36 通过。

S6 / 记忆搜索边界收口（2026-09-23）：移除记忆搜索内部输入的原生边框，保留搜索控件唯一的外层语义边界，避免出现双层内边框；四主题桌面 / 窄屏记忆搜索用例 8/8 通过。

S6 / 人物世界重复壳审计（2026-09-22）：移除通讯录面板重复的标题、说明、刷新和关闭栏，统一由人物世界外层身份头部与 tab 提供页面上下文；读取异常时保留就地重新读取入口。同步更新通讯录样张验收，避免产品体验层重复承担页面级导航职责。
