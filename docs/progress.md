...
- 2026-09-15：R10 正式添加向导已替换旧直接连接表单，测试 requestId 与窗口绑定，先真实测试再选工具保存；取消、超时、页面销毁清理，未保存连接不注册工具。正式 Renderer 8 项主题/宽度回归通过，主进程连接测试管理器已补配置锁、数量/分页/大小上限和保存后接管同一连接；仍缺新流程的独立 Electron 数据目录验收、OAuth 和异常断开恢复。
