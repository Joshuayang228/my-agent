...
- 基础组件工作台不再只展示按钮、输入和工具卡；新增标签切换、提示条、加载指示器、Markdown 渲染、资产目录、文件树和分栏拖拽等故事入口。
- 新增按钮与输入框 Foundation 注册项，注册表中的已建故事资产和候选资产全部进入基础资产总览。
- Markdown、文件树等正式基础能力使用真实组件 + 隔离样张验收；候选对话框、弹出层、下拉选择、进度条等只展示候选登记，不伪装为已实现。
