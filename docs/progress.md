...
- S4 草稿真实窗口边界已查明：普通关闭仅隐藏，重新显示保留名称和未保存 Key；应用内返回会拦截。重载与真正退出仍无提示丢弃草稿，未隐式保存连接；这是待修缺口，不能将隐私检查通过当作离开保护完成。本批只补隔离 Electron 测试及记录，生命周期 / 原诊断取消 2 项、Unit 1236、根 tsc / build 通过。RAG / OAuth 暂缓不变。
