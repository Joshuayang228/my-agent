...
- 共享 Markdown 已改为就近主题：代码高亮和 Mermaid 不再只跟随根主题；配置/绘制串行，保留安全策略及取消/失败清理。当前批次自审、807 项 Unit（串行）、根类型检查/build、85 项 UI、6 项真实 Electron、23 项 Framework Eval、1 项 Skill Eval 和资产检查通过；4 项外部模型测试按条件跳过。四主题宽窄与 Foundation 截图已检查，无新增独立 lint 命令；既有构建警告与 WISH-043/044 仍保留。
- 下一步：Foundation 全量复用核验、主进程类型门禁。Unix 终端仍需实机验证，完整 PTY 仍见 WISH-019，浏览器当前不是支持登录/脚本的完整浏览器。目标未完成。
