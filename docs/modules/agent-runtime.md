...
| Playground 模型连接与编程套餐候选 | 部分 | `SettingsExperienceCandidate.tsx` 读取六个套餐预设，名称 / 专用地址联动；官方、套餐、聚合、本地与自定义入口分离，适配器仅自定义可选。保存连接、添加模型、上方用途引用均为隔离状态；普通 Ark 的聚合分类只作用于候选。正式多连接 / 用途路由尚未回流，套餐后端验证见 `WISH-027` |
