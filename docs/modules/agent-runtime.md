...
- MCP 服务卡与添加向导的逐工具许可复选框共用 Foundation `CheckboxField`，基础故事也实际引用同一组件。保留原生标签 / Space / disabled 语义与固定 16px 操作槽；选中状态仍由正式许可数据或向导草稿持有，保存中禁用、失败保留，空选择仍提交空许可列表。不改变主进程授权和执行前校验，也不代表 OAuth 已接入。
