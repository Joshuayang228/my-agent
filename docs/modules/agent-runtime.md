...
| Playground 基础 / 产品体验工作台 | 已落地 | `src/components/playground/` · 基础 → 产品体验 → Agent 实验；统一内容宽度、单一一级入口、基础故事筛选和当前体验基础引用；图标尺寸 / 搜索 / 动效均有可见样张，边界见施工合同 | Playground Chat 已覆盖初次进入、聊天、处理中、确认、完成、失败六态；工作区只在处理中出现，确认 / 结果 / 失败均留在隔离对话流中。人物世界样张收敛为朋友圈 / 物什 / 名册，角色架改在设置样张查看；记忆归入设置的“记忆与相处”，业务状态不再作为独立入口。
