...
image_generate 按文件写入接受现有审批，不能因为是模型调用就视为只读。工具只接受主进程注入的已选项目 workspaceRoot，要求与 workdir 的真实路径一致；配置读取后、收费请求前以及最终提交前复核权限和目标。即使完全访问模式也不取消项目限定、拒绝符号链接及禁止覆盖已有文件的约束；请求期间出现同名文件时保留用户文件并失败，不自动重新生成。
...
对话审批入口由 ChatApprovalControl 共享，内部复用 Foundation ActionButton；正式写入仍走 settings.set executionMode 和主进程高风险确认，不在 Renderer 绕过确认。仅保存成功后回显新模式，失败 / 取消保留旧值并可重试；在途拒绝重复提交。菜单支持方向键、Home / End、Escape 和焦点回收，入口固定尺寸。Playground 仅展示禁用的同源入口，不改真实权限。

- 正式设置的默认审批方式与自定义规则保存使用独立在途计数，保存期间拒绝设置内切页、离开设置、窗口重载和退出；成功或失败均释放计数，失败由原表单保留草稿并重试，不自动重放权限请求。真实 Electron 验证规则写入后完整重启恢复。不承诺未提交草稿强退恢复，不改变权限引擎、校验或热更新行为。
...
| 自定义规则交互 | 已落地 | 正式设置与 Playground 共用 `PermissionRulesEditor`：默认收起；展开显示独立规则卡片，外层仅作无卡片折叠分组；添加入口位于列表下方，二次点击向下展开表单；固定尺寸入口原位切换“添加 / 取消添加”，hover / 取消不改变已有规则和入口位置；空白禁存；正式入口保存走 `settings:set('permissionRules')`，失败保留草稿和原规则 |
...
Loop 和 Debug 命中 ask 后必须明确确认；Registry 在执行前重查目标、规则和当次参数。确认凭据仅在主进程内保有对象身份，绑定会话及规则快照且一次性消费，不接受 Renderer 提供的 JSON 授权。内置权限目录从实际工具映射和优先级常量生成，责任链版本为 1.1.0。规则卡片与添加草稿已进入正式设置入口；Playground 仍只更新本地列表，不写真实权限 IPC。
