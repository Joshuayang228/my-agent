...
会话生成图片不是长期记忆：schema 17 的 messages.generated_images 保存工具媒体引用，由会话存储与受限媒体读取负责，不直接进入记忆检索。JSON 备份可以携带校验后的图片字节并恢复到应用专属目录，记忆正文仍沿现有预检与导入规则；媒体恢复不代表会话、记忆和设置已具备跨存储原子性，相关失败恢复继续由 R07 管理。
...
| 备份导入记忆路径 | 已落地 | `data:import` 整份预检复用 `memoryStore.assertMemoryContentAllowed`，正文须为 2～20,000 字符且不包含凭据，非法项在首次业务写入前拒绝整份备份；通过后仍走 `addMemory`，保留语义去重与向量写入。生活资产不进入记忆存储；合法输入的跨存储写入失败回滚仍是缺口 |
