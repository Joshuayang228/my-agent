...
朋友圈历史备份属于生活数据，不自动生成长期记忆或触发记忆抽取。它与已有记忆记录共用整份备份事务，但不进入记忆索引；记忆派生向量的崩溃重建仍由 WISH-045 跟踪。

会话生成图片不是长期记忆：schema 17 的 messages.generated_images 保存工具媒体引用，由会话存储与受限媒体读取负责，不直接进入记忆检索。JSON 备份携带校验后的图片字节并恢复到应用专属目录；会话、记忆、设置和生活资产共用一次同步 SQLite 事务及快照替换，写盘失败补偿内存。向量索引仍是成功后派生的异步任务，不纳入 SQLite 提交，也不承诺崩溃后自动补齐。
...
- 对外错误不暴露内部路径 / SQL
- 共用数据库快照写盘在临时写入 / rename 失败时保留旧文件并抛出友好错误，不再 copy 覆盖旧库；这是磁盘文件保障，不表示业务内存与向量双写已实现失败回滚。本次不改变记忆分类、召回和 CRUD 语义。
...
- 单条与批量 Embedding 请求在空 Key 时省略 Authorization，不发送空 Bearer；有 Key 时保留当前连接凭据。Electron 本地对话发现了额外 `/v1/embeddings` 请求并据此修复，`embedding-auth` Unit 覆盖两种请求。此改动不增加本地嵌入模型选择，不表示本地服务提供默认模型；RAG / 手动记忆向量同步的空 Key 门控仍单列缺口。
...
| 备份导入记忆路径 | 已落地 | 整份预检复用 `assertMemoryContentAllowed`，2～20,000 字符且拒绝凭据；与普通 `addMemory` 共用 `writeMemoryToDatabase`，保留语义去重、角色分桶。会话 / 生活资产 / 设置 / 记忆同步提交，SQL 与最终快照失败恢复；仅成功落盘后发布向量及资产副作用，统计实际新增数。向量崩溃补偿仍未实现 |
