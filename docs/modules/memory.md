...
| Playground 记忆管理候选 | 部分 | 设置“记忆”Tab 复用 `MemorySurface`，分类 Tab 与搜索入口同行；搜索默认是放大镜，展开后变成左侧放大镜、右侧清除的输入框；新增行跟随当前分类；编辑、删除和来源 Debug 样张仅写 Renderer 内存，正式 MemoryPanel、IPC、存储和 Prompt 不变 |
