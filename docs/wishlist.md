...
- WISH-045 本地向量能力（S2）：2026-09-20 已修复 RAG / 手动记忆同步的空 Key 门控和 Embedding 进程级永久失败状态，有 Key / 无 Key恢复均有独立 Electron 证据；仍需独立嵌入模型配置与连接切换后的索引一致性。当前默认 `text-embedding-3-small` 不能视为任意本地服务支持，镜像仅核对文本 / 分类 / 时间 / 角色、没有嵌入端点或模型身份；需避免不同向量空间混用。RAG 检索仍有两参数 queryItems 调用，需在同包补真实查询验证；不能将本地聊天协议或入口单测视为完整向量验收。
