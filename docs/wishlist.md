...
- WISH-045 R07 剩余恢复边界：备份 IPC 已有跨窗口互斥、发起窗口失效保护及同源记忆正文预检（非法输入整份零写入）；但合法输入的核心会话 / 生活资产提交后，记忆与设置仍分别异步写入，失败可能留下部分结果，未实现跨存储原子导入或崩溃恢复。需补完整事务 / 补偿方案与故障注入，不能把操作锁或预检当原子性。朋友圈互动备份仍按既有缺口管理。
