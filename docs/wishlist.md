...
- WISH-045 R07 剩余生命周期：本批共享数据页保留同一 SettingsPanel 内跨子页的忙碌锁及反馈隔离，但整个设置卸载 / 重新进入或多窗口可重新发起备份 IPC，data-export handler 尚无统一操作锁。后续需核验并收口主进程并发、关闭与失败恢复，不把按钮 disabled 当作全局并发保护；朋友圈互动备份仍按既有缺口管理。
