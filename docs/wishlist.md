...
- [ ] **WISH-041 · 工作区五功能正式回流（进行中）** — 2026-09-13 用户已授权目标模式回流；来源：`playground-workspace-five-tools-v1.md`。已接入 PanelRight、Foundation 标签和同源代码块、文件左树右多预览、审阅 unified／并排真实旧稿、受限只读浏览器，以及侧边聊天独立 workspace 会话／流式 Runtime／停止／清理。浏览器仍需评估完整导航、登录和站内交互是否进入产品范围；侧边聊天仍需上下文关联、工具确认隔离和完整失败恢复。终端后端须补早到输出／退出事件、调用者归属、进程树终止和关闭后清理失败的可靠回收，当前 kill 应答不能证明进程已退出。完整 PTY 仍见 WISH-019；Playground 形态与 Renderer IO 替身都不是后端证据。
