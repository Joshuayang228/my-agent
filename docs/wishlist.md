...
- [ ] **WISH-041 · 工作区五功能正式回流（进行中）** — 2026-09-13 用户已授权目标模式回流；来源：`playground-workspace-five-tools-v1.md`。已接入 PanelRight、Foundation 标签和同源代码块、文件左树右多预览，以及终端实例切换／关闭／跨页保持和项目隔离的 Renderer 生命周期。仍需补审阅并排与异步恢复、真实浏览器安全导航和侧边聊天运行时；终端后端须补早到输出／退出事件、调用者归属、进程树终止和关闭后清理失败的可靠回收，当前 kill 应答不能证明进程已退出。完整 PTY 仍见 WISH-019；21 种 Playground 形态与 Renderer IO 替身都不是后端证据。
