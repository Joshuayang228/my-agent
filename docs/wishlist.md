...
- WISH-045 确认流程收尾：本轮只完成 Skills 的确认失败恢复与 Foundation 固定操作槽；现有 AssetsPanel 删除与 CastPanel 强制召唤仍在异步结果前清除确认，MemoryPanel 敏感确认尚未绑定不可变草稿快照，CastPanel 还有字面换行转义文本。必须在对应模块逐一修复并补取消 / 失败 / 重复点击 / 改稿测试，不以共享 ConfirmPanel 的存在代替调用流程验收。
