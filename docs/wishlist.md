...
- WISH-045 生活面 / 备份补充：文化角、家居和足迹已共用展示组件与真实增改删；想去地点可按 footprint 写入。生活资产与播种标记已进入正式 `data:export` / `data:import`，按 id 合并并失败回滚。文化 starter 中的“留下 3 条笔记”“看过两次”等默认描述没有关联数量 / 事件证据，仍需复核角色内容来源，不能以写入数据库或备份往返代替人物设定授权。朋友圈互动仍不进入备份。
