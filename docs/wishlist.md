...
- WISH-045 MCP 回流补充：服务卡片已共用，正式添加向导仍为旧 stdio / SSE 表单；候选要求的 Streamable HTTP、Bearer 认证、测试后确认保存和可取消测试仍需补生产契约。不得将卡片共享或工具许可 IPC 通过视为整个 R10 完成；真实服务确认、异常断开和应用重启恢复仍需 Electron 证据。配置保存与连接停止仍是两个 IPC，保存后的停止通道异常只报告状态未确认并要求重启检查，后续须补主进程统一变更与状态恢复，不宣称原子操作。
