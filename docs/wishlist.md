...
- 工作区回流审计补充（WISH-041 / WISH-042）：共享 CodeBlock 的实际调用、原文与复制边界已补测试，五入口顺序已对齐；仍需验证四个候选的就近主题／Mermaid、审阅并排模式与异步错误／乱序恢复、关闭最后一个标签后重新打开、终端切换标签不卸载及早到输出。浏览器／侧聊的正式壳不能代替真实后端，整个目标继续进行。
...
- [ ] **WISH-041 · 工作区五功能正式回流（进行中）** — 2026-09-13 用户已授权目标模式回流；来源：`playground-workspace-five-tools-v1.md`。已接入 PanelRight 同页折叠保留、Foundation 标签和同源代码块；五入口顺序已统一但浏览器与侧聊只有未接入壳。文件左树右多预览已回流，实例切换／关闭／项目会话隔离仍需补齐，再补真实浏览器安全导航和侧边聊天运行时。正式文件与审阅已有 IPC，终端已有非 PTY 命令控制台，多实例事件过滤与卸载清理已补；完整 PTY 仍见 WISH-019；21 种 Playground 形态不是后端证据。
