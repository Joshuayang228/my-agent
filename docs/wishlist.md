...
- WISH-045 S4 [待确认] MCP 重叠写入：SettingsPanel 的新增按钮仅受 mcpAdding 禁用，已有服务卡 busy 仅跟随 mcpBusy；向导与已有服务管理可能同时工作。主进程已有配置锁，但整表旧快照是否覆盖向导新连接仍须真实入口复现，不能仅凭锁存在判定无冲突。本项是原跨入口竞争的具体检查点，不新增外围功能。
