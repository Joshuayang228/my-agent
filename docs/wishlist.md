...
- [ ] **WISH-041 · 工作区五功能正式回流** — 来源：`playground-workspace-five-tools-v1.md` 与用户五功能定位；重启条件：用户审阅 P0 并明确允许回流。核对正式审阅、文件、终端已有能力，补浏览器隔离导航与侧边聊天会话边界；同步真实生命周期、失败恢复和必要 IPC 契约。当前 21 种形态仅为隔离样张，不执行命令或请求模型，任务确认仍留在 Chat。
