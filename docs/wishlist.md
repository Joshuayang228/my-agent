...
- WISH-045 MCP OAuth 后续计划（2026-09-19 用户明确后置）：本次只要求主动登录 → 浏览器授权 → 回调 → MCP 连接与工具发现。跨重启加密凭据恢复、自动刷新 / 并发刷新、账户管理、更多供应商注册互操作与 SSE OAuth 后续按真实服务需求实施，不阻塞首次登录交付。当前会话取消、超时、回调 state / PKCE、防重放、凭据不泄露及退出清理仍必须在首次登录完成；首次登录本身尚未实现。
