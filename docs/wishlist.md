...
- WISH-045 R12 / R14 审计剩余差异：WorldSurface 的通讯录仍是静态姓名 / 关系行，正式 CastPanel 另有摘要、忙闲与召唤；需统一共享呈现与隔离样张，不能直接删真实能力或把静态关系当完整回流。正式 Moments / 衣柜空态仍暴露 tick、Catch-up 等内部术语，应按已确认产品文案层级清理。本批仅完成 WorldHub 基础导航复用，不提升全体验采用状态。
...
- WISH-042 生活资产写入批次全量 Unit 首跑：dev-server-watch.test.ts:35 在 mockClear 后收到额外 full-reload，1235 项通过、1 项失败。原代码原断言复核全量 1236 通过；根因未定位，不记为稳定性修复，归入既有 S5 收尾。
