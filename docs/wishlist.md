...
- WISH-045 R06 真实生图：用户已明确要求真实生成；2026-09-19 核查 image 仍由 `loadImageLLMConfig` 为附图理解选路由，工具 execute / ToolResult / tool_end 仅传文本且无生成工具注册。配置语义、真实协议调用、权限、图片落盘、结构化结果持久化及正式对话展示均需打通，完整边界已写入全产品施工合同「R06 真实生图链路」。不以相同 key、改标签或本地协议夹具认定完成，也不把 WISH-017 的人物自动生图暂缓扩大为用户主动生图排除。
