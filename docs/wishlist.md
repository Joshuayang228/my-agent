...
- WISH-045 R07 剩余恢复边界：核心 SQLite 各表与媒体的导入失败 / 进程崩溃一致性已接通，向量索引仍沿现有成功后异步派生；提交后立即退出或外部 Embedding 失败时尚无持久待办与启动重建，需要单独验证派生索引补偿，不能把核心数据恢复等同于向量双写一致。朋友圈互动备份仍按既有缺口管理；断电耐久性不在当前保障内。
