...
- [ ] **WISH-041 · 工作区五功能正式回流（进行中）** — 2026-09-13 用户已授权目标模式回流；来源：`playground-workspace-five-tools-v1.md`。已接入 PanelRight 同页折叠保留和共享 Foundation 标签；其它 Foundation 控件和五功能入口仍需统一；文件左树右多预览已回流，实例切换／关闭／项目会话隔离仍需补齐，再补真实浏览器安全导航和侧边聊天运行时。正式文件与审阅已有 IPC，终端已有非 PTY 命令控制台，多实例事件过滤与卸载清理已补；完整 PTY 仍见 WISH-019；21 种 Playground 形态不是后端证据。
