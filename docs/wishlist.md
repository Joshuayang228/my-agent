...
- WISH-045 S4 草稿离开保护已实测缺失：真实 Electron 的普通 close / show 保留未保存模型草稿，应用内返回也会拦截；page.reload 和 app.quit / 重启却不提醒且草稿消失，已保存连接未改变。下一步需补重载 / 显式退出的用户确认或留页保护，不能自动持久化未保存 Key，也不能把隐私断言通过记为修复。证据 s4-draft-lifecycle-final；仍属原 S4，不新增多窗口产品，RAG / OAuth 不恢复。
