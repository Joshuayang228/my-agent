...
- WISH-045 本地向量能力（S2）：2026-09-20 已修复 RAG / 手动记忆同步的空 Key 门控和 Embedding 进程级永久失败状态，有 Key / 无 Key恢复均有独立 Electron 证据；仍需独立嵌入模型配置与连接切换后的索引一致性。当前默认 `text-embedding-3-small` 不能视为任意本地服务支持，现已补请求空间指纹、返回模型与维度隔离，结构化源核对按空间重建；仍需独立配置、保存后唤醒和跨模型的文档 / 对话重建。无身份旧向量不再召回但不删源；服务在同端点、同名、同维度下静默换模无法凭现有协议识别，需可验证版本或显式重建。RAG 两参数 queryItems 与工具空 Key 门控已修复，真实 SQLite / Vectra 覆盖导入到工具查询及磁盘重开；仍不能将此视为完整向量配置验收。RAG 导入部分分块失败仍可能保存不完整文档，查询服务失败仍返回空结果，后续需在 S2 完整链核验中明确失败反馈与恢复，不能冒充全部文档已成功索引。
