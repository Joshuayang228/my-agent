...
- WISH-045 本地向量能力（S2）：2026-09-20 已修复 RAG / 手动记忆同步的空 Key 门控和 Embedding 进程级永久失败状态，有 Key / 无 Key恢复均有独立 Electron 证据；仍需独立嵌入模型配置与连接切换后的索引一致性。当前默认 `text-embedding-3-small` 不能视为任意本地服务支持，镜像仅核对文本 / 分类 / 时间 / 角色、没有嵌入端点或模型身份；需避免不同向量空间混用。RAG 两参数 queryItems 与工具空 Key 门控已修复，真实 SQLite / Vectra 覆盖导入到工具查询及磁盘重开；仍不能将此视为完整向量配置验收。RAG 导入部分分块失败仍可能保存不完整文档，查询服务失败仍返回空结果，后续需在 S2 完整链核验中明确失败反馈与恢复，不能冒充全部文档已成功索引。
