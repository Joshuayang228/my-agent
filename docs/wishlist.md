...
- WISH-045 R04 / S6：角色架旧导航子项已关闭（2026-09-21）：侧栏头像和通讯录快捷入口统一进入设置角色架，WorldHub / ShellView 不再保留 shelf 路由；宽窄 UI 返回原分区 2 项、真实 Electron 切角重载 1 项通过。全产品组合与其他入口审计继续，不涉及暂缓的 RAG / OAuth。
...
- WISH-045 S5 / S6：当前代码组合 UI 363、真实 Electron onboarding 21 项通过，无重试。合同清单已去除过时的“仍有硬编码 starter / 文档刷新 / 外部模型旧 UI”描述；onboarding 仍共用 beforeAll，未证明全部用例独立。其他 Electron 文件及逐项采用审计继续，RAG / OAuth 后续维持暂缓。

- WISH-045 R01 / R14：审批入口已共享，成功后回显、失败重试和候选隔离通过定向 UI 17、Unit 1241 与根 tsc；覆盖此前“审批菜单未共享”的历史待办。真实 Electron 高风险确认和全产品组合审计仍需最终验收，四项体验不提升 adopted；RAG / OAuth 后续保持暂缓。

- WISH-045 R01 / R14：消息外框已共享并通过定向 UI 12、Unit 1240 与根 tsc / build；覆盖下方历史“消息流外框未统一”状态。审批菜单和全产品正式组合验收仍待推进，不能用共享外框完成替代整体采用；RAG / OAuth 后续继续暂缓。


- WISH-045 R01 / R14：ChatComposer 已接正式与候选，欢迎区和输入卡片不再各写一套。消息流头像 / 名称 / 正文组合仍独立，审批菜单也尚未纳入共享呈现；继续沿真实引用、工具卡、编辑、复制及发送状态核验，不能复制候选固定回复与“刚刚”时间。全产品目标未关闭，RAG / OAuth 后续暂缓。

- WISH-045 R01 / R14：欢迎区已统一为 ChatWelcome + Foundation ActionButton，正式与候选各注入真实 / 隔离回调；7 项 UI、Unit 1238、根 tsc / build 通过。审计仍见 Chat 输入区与消息流各自维护 JSX，须核验共享呈现及真实交互边界，不能凭欢迎区采用关闭整项 Chat 或全产品目标。RAG / OAuth 后续维持暂缓。

- WISH-045 S4 MCP 向导 / 管理重叠覆盖已复现并修复：旧列表删除提交 [] 会覆盖向导新连接，现两个入口互斥；UI 10、Unit 1237、根 tsc / build 通过。只关闭这一具体路径，其余生命周期与全产品 S6 仍待最终验收，不恢复 RAG / OAuth 后续。

- WISH-045 S6 人物头部共享回流已接通：WorldProfileHeader 接 App 当前角色名称 / 简介，候选共用组件并保持隔离；主题宽窄与候选切角验证通过。全产品组合验收仍未关闭，不把头部或六标签统一视为整页最终采用。

- WISH-045 新增向导真实 Electron 保存 / 重载 / 退出保护与完整重启证据已补齐，见质量总控 mcp-wizard-desktop。本条关闭该验收子项，覆盖下方历史待核验语义；跨入口写入竞争及全产品最终验收仍待收口，未提交草稿不自动保存，RAG / OAuth 后续暂缓。

- WISH-045 S4 MCP 新增向导 Renderer 保存 / 刷新保护已实现并通过 12 项 UI 回归，测试 / 取消阶段仍允许离页并清理。普通向导真实 Electron 保存 / 重载 / 退出 / 重启和跨入口竞争尚未由本批验证；未提交草稿不自动保存，OAuth 后续继续暂缓。

- WISH-045 S4 MCP 普通管理在途保护已落地：停用 / 删除 / 工具许可红测转绿，UI 13、真实 Electron 删除配置与权限保存生命周期 2、Unit 1237、根 tsc / build 通过。新增连接向导的保存 / 卸载、未提交草稿及跨入口写入竞争仍需核验；OAuth 启停 / 重试维持原行为，后续保持暂缓，不关闭总目标。
