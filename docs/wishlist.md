...
- WISH-045 R12 / R14 审计剩余差异：WorldSurface 通讯录仍是静态姓名 / 关系行，正式 CastPanel 有摘要、忙闲与召唤，需统一共享呈现与隔离样张，不能删真实能力或把静态关系当完整回流。WorldHub 基础导航及 Moments / 衣柜内部术语已清理并验证；通讯录差异仍未关闭，不提升全体验采用状态。
