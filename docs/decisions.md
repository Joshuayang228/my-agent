...
> 配置载体已由 DEC-042 的用途路由策略取代：辅助场景仍独立配置，但不再读取 auxModel；历史选项保留作取舍记录。
...
### DEC-042：开发期回流不承担旧界面与旧数据兼容

- **日期**：2026-09-16
- **状态**：已决定，用户明确授权；延续并扩大 DEC-035 的开发期直接重置策略。
- **决定**：正式产品以已确认的 Playground 体验为基线。旧设置能力、旧接口和旧开发数据不构成保留旧 UI 或增加迁移层的理由；冲突时可直接替换结构并重建相关开发数据。
- **边界**：不是批量删除无关文件、凭据、其他项目或未参与变更数据的指令；权限、路径安全和凭据存储仍需完整实现。功能成功后产生的新数据，仍必须具备真实持久化和失败恢复能力。
- **影响**：全产品回流施工合同中的旧兼容验收要求由本决策覆盖；只有明确要对已发布版本 / 外部用户负责时，才重新讨论兼容或迁移策略。
- **模型配置落点**：连接与用途是唯一身份来源，删空和无有效主用途不恢复旧全局字段或环境配置；辅助 / 图片未独立安排时沿用新主用途。这取代 DEC-014 的 auxModel 载体，不取消辅助任务独立配置。显式单次测试覆盖仍保留；不将无兼容要求扩大为批量删除存储键或其他配置。

### DEC-043：MCP OAuth 本次只交付首次登录闭环

- **日期**：2026-09-19
- **状态**：已决定，用户明确要求缩小范围。
- **决定**：复用 MCP SDK 的发现、公共客户端注册、PKCE 与授权码交换；令牌只驻留当前主进程，授权后沿测试连接 / 选择工具 / 保存接管流程启用。仅用户主动操作可打开系统浏览器；重启或认证失效后要求重新登录。
- **边界**：不实现跨重启登录恢复、自动刷新、多账户及 SSE OAuth。这些后续计划归 WISH-045；state / 回调单次消费、取消、超时、窗口归属、网络限制与凭据隔离仍属于本次必要安全边界，不能延后。
- **影响**：MCP 授权模块、传输工厂、连接管理、IPC 四处契约、共享表单与服务卡及真实协议 / Electron 验收；不改变工具许可和高风险审批规则。不以本地协议测试宣称第三方服务全部兼容。
