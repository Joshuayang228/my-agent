...
- 正式 Right Dock 的侧边聊天不复用主会话：`session:createWorkspace` 创建 `session_kind=workspace` 的临时 Runtime 会话，`listSessions` 排除它，面板卸载删除它；`chat:event` 必须按该 sessionId 过滤，停止调用该会话的 `chat:abort`。工作区会话不得被误当作用户主会话、导览入口或永久历史。
