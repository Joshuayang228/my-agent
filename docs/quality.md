...
Debug 入口与壳变化时，Renderer E2E 必须验证侧栏进入全页 `DevPanel`、Chat 不出现 `conversation-debug-toggle` 或 `ConversationDebugAside` 半屏、默认显示只读“运行概览”、概览证据入口可达，以及 Debug 内部分区仍可切换；右坞回归只验证文件 / 预览 / 审阅 / 终端，不得重新引入 Debug 覆盖。
