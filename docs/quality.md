...
S6 / 朋友圈卡片密度门禁（2026-09-23）：正式朋友圈回归验证动态卡片按内容高度收缩，操作区仍位于卡片内容之后；卡片高度断言与点赞 / 评论交互同轮通过。Playground 角色样张文案不作为本轮正式朋友圈证据。
...
朋友圈操作槽基础复用（2026-09-21）：正式朋友圈刷新、关闭与动态选项接入 Foundation IconButton；根 `npx tsc --noEmit` 通过。既有真实赞评 / 评论槽几何回归继续覆盖数据链路，本批未改 IPC、存储、RAG 或 OAuth。
...
S5 协议验收入口独立化（2026-09-20）：单独运行 onboarding 自定义协议用例时，首屏已在设置，旧首句点击隐藏侧栏按钮超时。现等待启动后按设置实际可见性进入；首次保存等待遵循生产安全存储 15 秒上限，局部断言预算 20 秒。失败清理仅销毁独占测试 Electron 窗口，避免草稿保护导致 teardown 超时，不改产品 beforeunload。单独完整重启协议用例通过（11.7 秒），整组 onboarding 21 项通过（57 秒），Unit 1237 通过。electron.test.ts 已使用新连接表单和逐例独立目录，旧清单中的旧 UI 描述不再成立；本批不调用外部模型，不把其余 onboarding 用例宣称为全部独立。 证据：var/verification/onboarding-entry-red、onboarding-entry-fixed、onboarding-entry-final、onboarding-entry-suite。
...
S6 生活面切角门禁：Renderer 三用例持有旧读取和新增草稿，广播切角后要求三页展示新角色、草稿清空，释放旧请求后不能回退；四主题宽窄与新增失败回归合计 12 项通过（s6-living-role-final）。首次定位错用“名称”标签的批次已中止；更正为真实作品 / 物件 / 地点标签后取得三条红测（s6-living-role-red-verified）。真实 Electron 新用例不借用前序数据：主动提交本地模型配置，在独立临时 userData 创建两个角色各四类资产，调用真实 requestSwitch、接收广播、从正式四页检查更新，重载后核对资产 ID，清理自身数据并恢复原角色。独立 1 项、完整 onboarding 20 项通过；Unit 1236、根 tsc / build 通过。无 IPC / 依赖变更，无独立 lint 脚本，已有构建警告保留；不宣称主进程类型全绿或资产在途写入竞争已验收。
