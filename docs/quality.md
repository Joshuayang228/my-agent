...
文件规则门禁使用真实临时文件和正式 Registry：验证写入 / 编辑 / 补丁 / 删除 / 读取、别名、隐式补丁目标、目录子项与扫描上限、symlink、越界、无交互拒绝、确认后参数 / 规则 / 会话 / callId 变化及重复确认。Loop 只替换模型输出，文件执行与权限不替换；Electron 用独立数据目录验证真实 settings IPC 热更新、原生确认结果、文件副作用和完整重启载入。上述后端证据不替代 R08 规则卡片和新增表单的正式入口验收。
...
MCP 服务卡片 Unit 覆盖未知 / 零工具、文本转义、内部滚动和无回调不生成操作；正式 Renderer 从 App 设置入口覆盖四主题宽窄、连接失败重试、忙时固定操作槽、许可保存失败、状态读取失败恢复，以及禁用 / 删除保存失败不得先断开。Playground 场景回归验证同一组件的确认、停用与取消；Renderer IPC 替身不能替代真实远程服务、主进程确认或重启恢复证据。
...
MCP 工具许可回归覆盖旧配置兼容、工具名唯一性/长度校验、设置页真实工具清单入口以及 Registry/执行前双重过滤；真实连接、认证取消和重连仍需 Electron 服务证据。
