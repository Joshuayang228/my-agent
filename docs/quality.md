...
UI 组件 / 图标语义注册表、Playground 目录或故事发生变化时，Unit 必须验证稳定 key、分类、来源和生命周期，Renderer 必须运行 `npm run test:e2e` 验证入口与筛选；不改变 Agent 行为契约时，不要求运行真实 Persona Eval。Foundation / Experience 边界变化时，Unit 还必须验证产品体验的 `experienceParts`、`usesFoundation`、层级和生命周期约束；Foundation 工作台必须覆盖所有已建故事和所有候选资产；E2E 必须验证业务 Tab 可切换且 Playground fixture 不触发真实 IPC。
