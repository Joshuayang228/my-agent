...
- Skills / MCP 候选还必须验证 Settings 下两个一级入口、Skills 主行只保留名称与启用开关、点击名称进入详情、作者 / 版本 / 来源 / 状态展示、Skill 文件树与内容预览切换、返回列表，以及 MCP 来源到工具选择的向导、取消 / 保存状态；工作区候选必须验证默认态不打开右坞、文件任务按需出现、审阅不承载确认、完成结果回到 Chat，且所有交互不调用生产 IPC。
