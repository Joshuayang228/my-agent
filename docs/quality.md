...
- Playground 记忆 / 设置关系旅程的 Renderer E2E 必须验证默认记忆样张显示“来自 / 之后会”的关系证据且不泄露向量、Prompt、召回分数等内部词；“纠正记忆”必须可在 Renderer 内存夹具中实际保存；从记忆进入设置时必须选中记忆管理场景和 Settings「记忆」分区，能返回记忆并回到 Chat；预览不得触发真实 memory / settings IPC 或改变正式默认分区。
