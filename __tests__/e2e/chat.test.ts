...
test.describe('My Agent UI', () => {
  ...
  test('Playground 基础与产品体验分层且可独立验收', async ({ page }) => {
    ...
    await expect(page.getByTestId('memory-scope-user')).toHaveAttribute('aria-selected', 'true')
    await expect(page.getByTestId('memory-scope-user')).toContainText('6')
    await expect(memorySurface.getByTestId('memory-preview-evidence-memory-user-workflow')).toContainText('来自：')
    await expect(memorySurface.getByTestId('memory-preview-evidence-memory-user-workflow')).toContainText('之后会：')
    ...
    await page.getByTestId('memory-scope-relationship').click()
    await expect(page.getByTestId('memory-scope-relationship')).toHaveAttribute('aria-selected', 'true')
    await expect(page.getByTestId('memory-scope-relationship')).toContainText('6')
    await expect(memorySurface).toContainText('我们先参考 Alice 和项目现状，再决定是否自己设计。')
