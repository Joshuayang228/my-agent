...
test.describe('My Agent UI', () => {
  ...
  test('Playground 基础与产品体验分层且可独立验收', async ({ page }) => {
    ...
    const nav = page.locator('[data-testid="playground-shell"] nav')
    ...
    const primarySidebarAsset = page.locator('article').filter({ hasText: 'layout.primary-sidebar' })
    await expect(primarySidebarAsset).toContainText('被产品体验使用')
    await expect(primarySidebarAsset).toContainText('Chat')
    ...
    await expect(chatDependencies).toContainText('experience.chat')
    await expect(chatDependencies.getByLabel('使用的基础组件')).toContainText('主侧栏')
    await expect(chatDependencies.getByLabel('使用的基础组件')).toContainText('空状态')
    ...
    await expect(workspaceDependencies).toContainText('experience.workspace')
    await expect(workspaceDependencies.getByLabel('使用的基础组件')).toContainText('右侧工作坞')
    await expect(workspaceDependencies.getByLabel('使用的基础组件')).toContainText('文件树')
  ...
  test('Playground Toast 四态关闭按钮沿统一右边界对齐', async ({ page }) => {
    await page.goto('/')
    await page.locator('[data-testid="primary-sidebar"]').getByRole('button', { name: 'Playground', exact: true }).click()
    await page.locator('[data-testid="playground-shell"] nav').getByRole('button', { name: '业务状态', exact: true }).click()
  ...
  test('Debug 与 Playground 采用一级任务导航', async ({ page }) => {
    ...
    await expect(playgroundNav.getByRole('button', { name: '设计令牌', exact: true })).toBeVisible()
