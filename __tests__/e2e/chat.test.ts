...
test.describe('My Agent UI', () => {
  ...
  test('Playground 设置候选覆盖新的信息架构与隔离交互', async ({ page }) => {
    ...
    await expect(candidate.getByLabel('中转 / 聚合服务', { exact: true })).toBeVisible()
    await expect(candidate.getByLabel('中转 / 聚合服务', { exact: true })).toContainText('OpenRouter')
    await candidate.getByLabel('中转 / 聚合服务', { exact: true }).selectOption('openrouter')
