...
test.describe('My Agent UI', () => {
  ...
  test('Playground 设置候选覆盖新的信息架构与隔离交互', async ({ page }) => {
    ...
    const modelOptions = candidate.getByLabel('选择已有模型 OpenAI 主账号', { exact: true })
    await expect(modelOptions).toBeVisible()
    await modelOptions.selectOption('o3-mini')
    await expect(candidate.getByLabel('手动添加模型 OpenAI 主账号', { exact: true })).toHaveValue('o3-mini')
    await candidate.getByRole('button', { name: '手动添加模型', exact: true }).click()
  ...
  for (const theme of ['dark', 'light']) {
    for (const width of [1165, 520]) {
      test(`Playground 连接卡片头部操作与模型下拉 ${theme} ${width}`, async ({ page }, testInfo) => {
        ...
        await expect(select.locator('option[value="gpt-4o"]')).toBeDisabled()
        await select.selectOption('o3-mini')
        await expect(input).toHaveValue('o3-mini')
        await expect(profile).toContainText('模型清单 · 3 个')
        await expect(profile.getByRole('button', { name: 'o3-mini', exact: true })).toHaveCount(0)
        await add.click()
        await expect(input).toHaveValue('')
        await expect(profile).toContainText('模型清单 · 4 个')
        await expect(select.locator('option[value="o3-mini"]')).toBeDisabled()
        ...
        await other.getByTestId('settings-candidate-connection-header-connection-openrouter-1').getByRole('button', { expanded: false }).click()
