...
test.describe('My Agent UI', () => {
  ...
  test('Playground 基础与产品体验分层且可独立验收', async ({ page }) => {
    ...
    await expect(page.getByTestId('memory-overview')).toContainText('已记住')
    await expect(page.getByTestId('memory-overview')).toContainText('可纠正、可删除')
    await page.getByTestId('memory-add-button').click()
    const memoryAddForm = page.getByTestId('memory-add-form')
    await memoryAddForm.getByLabel('新记忆内容', { exact: true }).fill('我希望复杂任务先给结论，再展开步骤。')
    await memoryAddForm.getByRole('radio', { name: '沟通偏好', exact: true }).click()
    await memoryAddForm.getByRole('button', { name: '保存记忆', exact: true }).click()
    ...
    await page.getByLabel('搜索记忆', { exact: true }).fill('复杂任务')
    await expect(memorySurface).toContainText('我希望复杂任务先给结论，再展开步骤。')
    ...
    await memorySurface.locator('input').last().fill('我希望复杂任务先给结论，再展开关键步骤。')
    ...
    await expect(memorySurface.getByTestId('memory-sensitive-warning-memory-sensitive')).toContainText('敏感信息')
    await expect(memorySurface.getByTestId('memory-sensitive-warning-memory-sensitive')).toContainText('请确认是否需要长期保留')
  ...
  test('Playground 设置候选覆盖新的信息架构与隔离交互', async ({ page }) => {
    ...
    await expect(candidate.getByTestId('settings-candidate-section-skills')).not.toContainText('帮助 Agent 归纳文件')
    await candidate.getByTestId('settings-candidate-skill-open-detail').click()
    const skillDetail = candidate.getByTestId('settings-candidate-skill-detail')
    await expect(skillDetail).toContainText('作者')
    await expect(skillDetail).toContainText('v1.2.0')
    await expect(skillDetail.getByTestId('settings-candidate-skill-file-preview')).toContainText('文件整理助手')
    await skillDetail.getByTestId('settings-candidate-skill-file-examples-basic-md').click()
    await expect(skillDetail.getByTestId('settings-candidate-skill-file-preview')).toContainText('# 示例')
    await skillDetail.getByTestId('settings-candidate-skill-back').click()
    await expect(candidate.getByTestId('settings-candidate-skill-file-organizer')).toBeVisible()
