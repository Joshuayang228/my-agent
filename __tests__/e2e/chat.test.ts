...
for (const theme of ['porcelain-blue', 'yao-stone', 'song-smoke', 'deep-plum']) {
  for (const width of [1166, 600]) {
    test(`正式模型高级区同源与失败恢复 ${theme} ${width}`, async ({ page }, testInfo) => {
      await page.setViewportSize({ width, height: 731 })
      await installProductionElectronStub(page)
      await page.addInitScript((theme) => {
        localStorage.setItem('theme', theme)
        const api = (window as any).electronAPI
        const connections = [{ id: 'primary', name: '主连接', baseUrl: 'http://127.0.0.1:11434/v1', provider: 'openai', model: 'primary-model', enabled: true, models: [{ id: 'primary-model', enabled: true }] }]
        const values: Record<string, string> = { llmConnectionReady: 'true', modelConnections: JSON.stringify(connections), modelRoutes: JSON.stringify([{ purpose: 'primary', connectionId: 'primary', model: 'primary-model', enabled: true }]), llmTemperature: '0.7', sessionTokenBudget: '0', dailyTokenBudget: '0' }
        const state = { values, writes: [] as unknown[], failSave: false, tested: [] as unknown[], pending: [] as Array<{ resolve: (value: unknown) => void; reject: (error: Error) => void }> }
        ;(window as any).__advanced = state
        api.settings.get = async () => ({ ...values })
        api.settings.set = async (key, value) => { if (state.failSave) throw new Error('private save error'); state.writes.push([key, value]); values[key] = value }
        api.settings.saveModelConfiguration = async (input) => { values.modelConnections = input.connections; values.modelRoutes = input.routes }
        api.settings.testConnection = (input) => { state.tested.push(input); return new Promise((resolve, reject) => state.pending.push({ resolve, reject })) }
      }, theme)
      await page.goto('/')
      await page.getByTestId('primary-sidebar').getByRole('button', { name: '设置', exact: true }).click()
      const settings = page.getByTestId('settings-panel')
      const navigate = (name: string) => settings.getByRole(width < 768 ? 'tab' : 'button', { name, exact: true }).click()
      await navigate('模型')
      const advanced = page.getByTestId('settings-model-advanced')
      const toggle = advanced.getByTestId('settings-model-advanced-toggle')
      await expect(toggle).toHaveAttribute('aria-expanded', 'false')
      await expect(advanced.locator('input')).toHaveCount(0)
      await toggle.click()
      await expect(advanced.locator('input')).toHaveCount(3)
      await expect(page.getByLabel('Top P', { exact: true })).toHaveCount(0)
      await expect(page.getByLabel('最大输出 Token', { exact: true })).toHaveCount(0)
      const budget = advanced.getByLabel('会话预算（Token）', { exact: true })
      const temperature = advanced.getByLabel('Temperature', { exact: true })
      await budget.fill('-1')
      await expect(budget).toHaveAttribute('aria-invalid', 'true')
      await navigate('外观与界面')
      await expect(advanced).toBeVisible()
      expect(await page.evaluate(() => (window as any).__advanced.writes)).toEqual([])
      await budget.fill('12000')
      await temperature.fill('3')
      await navigate('外观与界面')
      await expect(temperature).toHaveValue('3')
      expect(await page.evaluate(() => (window as any).__advanced.values.llmTemperature)).toBe('0.7')
      await page.evaluate(() => { (window as any).__advanced.failSave = true })
      await temperature.fill('0')
      await navigate('外观与界面')
      await expect(advanced.getByRole('alert')).toContainText('设置未保存')
      await expect(temperature).toHaveValue('0')
      await page.evaluate(() => { (window as any).__advanced.failSave = false })
      await advanced.getByRole('button', { name: '重试保存', exact: true }).click()
      await expect.poll(() => page.evaluate(() => (window as any).__advanced.values.llmTemperature)).toBe('0')
      await expect(advanced.getByRole('alert')).toHaveCount(0)
      const testButton = advanced.getByTestId('settings-model-test')
      await testButton.scrollIntoViewIfNeeded()
      const size = () => testButton.evaluate(node => ({ width: node.getBoundingClientRect().width, height: node.getBoundingClientRect().height }))
      ...
      expect(await size()).toEqual(initialSize)
      expect(await page.evaluate(() => (window as any).__advanced.tested[0])).toEqual({ connectionId: 'primary', useStoredApiKey: true, baseUrl: 'http://127.0.0.1:11434/v1', model: 'primary-model', provider: 'openai' })
      await page.evaluate(() => (window as any).__advanced.pending.shift().reject(new Error('private IPC failure')))
      await expect(advanced.getByRole('status')).toContainText('连接测试未完成，请重试')
      await expect(advanced).not.toContainText('private IPC failure')
      expect(await size()).toEqual(initialSize)
      await testButton.click()
      await page.evaluate(() => (window as any).__advanced.pending.shift().resolve({ ok: true, model: 'primary-model', ms: 1 }))
      await expect(advanced.getByRole('status')).toContainText('连接测试通过')
      expect(await size()).toEqual(initialSize)
      expect(await advanced.evaluate(node => node.scrollWidth <= node.clientWidth)).toBe(true)
      await advanced.screenshot({ path: testInfo.outputPath('model-advanced.png') })
      await testButton.click()
      await page.getByRole('button', { name: '编辑连接 主连接', exact: true }).click()
      await page.getByTestId('model-connection-form').getByLabel('API Key', { exact: true }).fill('test-replaced-key')
      await page.getByTestId('model-connection-form').getByRole('button', { name: '保存连接', exact: true }).click()
      await expect(testButton).toBeEnabled()
      await page.evaluate(() => (window as any).__advanced.pending.shift().resolve({ ok: true, model: '旧密钥迟到结果', ms: 1 }))
      await expect(advanced.getByRole('status')).toHaveCount(0)
      await testButton.click()
      await page.getByTestId('settings-route-primary').getByRole('switch').click()
      await expect(advanced).toContainText('请先为主对话安排模型')
      await page.evaluate(() => (window as any).__advanced.pending.shift().resolve({ ok: false, error: '旧连接迟到结果' }))
      await expect(advanced.getByRole('status')).toHaveCount(0)
      await expect(testButton).toBeDisabled()
      await navigate('外观与界面')
      await navigate('模型')
      ...
      await expect(page.getByLabel('会话预算（Token）', { exact: true })).toHaveValue('12000')
    })
...
for (const theme of ['porcelain-blue', 'yao-stone', 'song-smoke', 'deep-plum']) {
  for (const width of [1166, 600]) {
    test(`正式连接卡片请求状态操作槽不变 ${theme} ${width}`, async ({ page }, testInfo) => {
      await page.setViewportSize({ width, height: 731 })
      await installProductionElectronStub(page)
      await page.addInitScript(theme => {
        localStorage.setItem('theme', theme)
        const model = 'long-model-'.repeat(18)
        const values = { llmConnectionReady: 'true', llmModel: model,
          modelConnections: JSON.stringify([{ id: 'geometry', name: '长连接名称'.repeat(16), baseUrl: 'https://fixture.test/v1', source: 'custom', provider: 'openai', enabled: true, hasApiKey: true, model, models: [{ id: model, enabled: true }] }]), modelRoutes: '[]' }
        const state = { values, model, testCalls: 0, fetchCalls: 0, test: null as null | ((result: unknown) => void), fetch: null as null | ((result: unknown) => void) }
        ;(window as any).__connectionGeometry = state
        const api = (window as any).electronAPI
        api.settings.get = async () => ({ ...values })
        api.settings.saveModelConfiguration = async (input: { connections: string; routes: string }) => { values.modelConnections = input.connections; values.modelRoutes = input.routes }
        api.settings.testConnection = () => { state.testCalls++; return new Promise(resolve => { state.test = resolve }) }
        api.settings.fetchModels = () => { state.fetchCalls++; return new Promise(resolve => { state.fetch = resolve }) }
      }, theme)
      await page.goto('/')
      await page.getByTestId('primary-sidebar').getByRole('button', { name: '设置', exact: true }).click()
      await page.getByTestId('settings-panel').getByRole(width < 768 ? 'tab' : 'button', { name: '模型', exact: true }).click()
      const profile = page.getByTestId('settings-model-profile-geometry')
      const header = page.getByTestId('settings-connection-header-geometry')
      await header.scrollIntoViewIfNeeded()
      const geometry = () => header.getByRole('button').evaluateAll(nodes => nodes.map(node => { const { x, y, width, height } = node.getBoundingClientRect(); return { x, y, width, height } }))
      ...
      await expect(testButton).toBeDisabled()
      expect(await geometry()).toEqual(before)
      await page.evaluate(() => { const state = (window as any).__connectionGeometry; state.test({ ok: true, model: state.model, ms: 1 }) })
      await expect(profile).toContainText('连接测试通过')
      expect(await geometry()).toEqual(before)
      await testButton.click()
      await page.evaluate(() => (window as any).__connectionGeometry.test({ ok: false, error: '连接测试失败，请重试' }))
      await expect(profile).toContainText('连接测试失败，请重试')
      expect(await geometry()).toEqual(before)
      await fetchButton.click()
      await expect(fetchButton).toBeDisabled()
      expect(await geometry()).toEqual(before)
      await page.evaluate(() => (window as any).__connectionGeometry.fetch({ ok: false, error: '获取失败，请重试', reason: 'network', retryable: true }))
      await expect(profile).toContainText('获取失败，请重试')
      expect(await geometry()).toEqual(before)
      await fetchButton.click()
      await page.evaluate(() => { const state = (window as any).__connectionGeometry; state.fetch({ ok: true, models: [state.model, `${state.model}extra`] }) })
      await expect(profile.getByTestId('settings-fetched-models-geometry')).toBeVisible()
      expect(await geometry()).toEqual(before)
      expect(await profile.evaluate(node => node.scrollWidth <= node.clientWidth + 1)).toBe(true)
      await profile.screenshot({ path: testInfo.outputPath('connection-card.png') })
      const fetched = profile.getByTestId('settings-fetched-models-geometry')
      await expect(fetched.getByRole('button').first()).toBeDisabled()
      await fetched.getByRole('button').nth(1).click()
      await expect(profile.locator('[data-connection-model-row]')).toHaveCount(2)
      const extra = profile.locator('[data-connection-model-row]').nth(1)
      await extra.getByRole('switch').click()
      await expect(extra.getByRole('switch')).toHaveAttribute('aria-checked', 'false')
      await extra.getByRole('button', { name: /^移除模型 / }).click()
      await expect(profile.locator('[data-connection-model-row]')).toHaveCount(1)
      await profile.getByRole('button', { name: /^编辑连接 / }).click()
      await expect(page.getByTestId('settings-model-add')).toBeDisabled()
      await profile.getByRole('button', { name: '取消', exact: true }).click()
      await expect(header).toBeVisible()
      expect(await page.evaluate(() => { const state = (window as any).__connectionGeometry; return [state.testCalls, state.fetchCalls] })).toEqual([2, 2])
      const list = page.getByTestId('settings-model-connections')
      const listHeading = page.getByTestId('settings-model-connections-heading')
      const add = page.getByTestId('settings-model-add')
      await listHeading.scrollIntoViewIfNeeded()
      const headingBox = await listHeading.getByRole('heading', { name: '连接与模型清单' }).boundingBox()
      const addBox = await add.boundingBox()
      expect(addBox!.x).toBeGreaterThan(headingBox!.x + headingBox!.width)
      expect(addBox!.y).toBe(headingBox!.y)
      await add.hover()
      expect(await add.boundingBox()).toEqual(addBox)
      await add.click()
      ...
      await expect(list).toContainText('还没有连接，添加后再维护模型清单。')
      expect(await list.evaluate(node => node.scrollWidth <= node.clientWidth + 1)).toBe(true)
      expect(await page.evaluate(() => JSON.parse((window as any).__connectionGeometry.values.modelConnections))).toEqual([])
      await add.click()
...
test('候选用途安排与连接卡片操作只修改隔离状态', async ({ page }) => {
  await installProductionElectronStub(page)
  await page.addInitScript(() => {
    const api = (window as any).electronAPI
    const get = api.settings.get
    ...
    api.settings.testConnection = async () => { (window as any).__candidateRouteWrites++; return { ok: true } }
    api.settings.fetchModels = async () => { (window as any).__candidateRouteWrites++; return { ok: true, models: [] } }
  })
  await page.goto('/')
  await page.getByTestId('primary-sidebar').getByRole('button', { name: 'Playground', exact: true }).click()
  await page.getByTestId('playground-nav').getByRole('button', { name: '设置', exact: true }).click()
  ...
  await profile.getByTestId('settings-candidate-fetch-models-connection-openai-0').click()
  await expect(profile.getByTestId('settings-candidate-fetched-models-connection-openai-0')).toBeVisible()
  await profile.getByTestId('settings-candidate-test-connection-connection-openai-0').click()
  await expect(profile).toContainText('连接测试通过（样张）')
  await profile.getByRole('textbox', { name: '手动添加模型 OpenAI 主账号', exact: true }).fill('isolated-model')
  await profile.getByRole('button', { name: '手动添加模型', exact: true }).click()
  await profile.getByRole('switch', { name: 'isolated-model已启用', exact: true }).click()
  await expect(profile.getByRole('switch', { name: 'isolated-model未启用', exact: true })).toBeVisible()
  await profile.getByRole('button', { name: '移除模型 isolated-model', exact: true }).click()
  await expect(profile.getByRole('switch', { name: /isolated-model/ })).toHaveCount(0)
  await candidate.getByTestId('settings-candidate-model-advanced-toggle').click()
  await candidate.getByLabel('Temperature', { exact: true }).fill('0')
  await candidate.getByLabel('会话预算（Token）', { exact: true }).fill('12000')
  await candidate.getByTestId('settings-candidate-model-test').click()
  await expect(candidate.getByTestId('settings-candidate-model-status')).toContainText('连接测试通过')
...
test('正式模型请求结果跟随连接身份，拒绝迟到结果并恢复异常', async ({ page }) => {
  ...
  await form.getByLabel('API Key', { exact: true }).fill('fixture-replacement')
  await form.getByRole('button', { name: '保存连接', exact: true }).click()
  await expect(profile.getByRole('img', { name: '尚未测试', exact: true })).toBeVisible()
...
test('MCP 共享表单取消迟到结果、字段失效和重测', async ({ page }, testInfo) => {
  ...
  await expect(page.getByTestId('settings-mcp-add')).toBeDisabled()
...
for (const theme of ['porcelain-blue', 'yao-stone', 'song-smoke', 'deep-plum']) {
  for (const width of [1166, 600]) {
    test(`正式 MCP 共享服务卡与操作恢复 ${theme} ${width}`, async ({ page }, testInfo) => {
      ...
      await expect(page.getByTestId('settings-mcp-add')).toBeEnabled()
    ...
    test(`正式 MCP 添加连接先测后存 ${theme} ${width}`, async ({ page }) => {
      ...
      await page.getByTestId('primary-sidebar').getByRole('button', { name: '设置', exact: true }).click()
      await page.getByRole(width < 640 ? 'tab' : 'button', { name: 'MCP', exact: true }).click()
      const add = page.getByTestId('settings-mcp-add')
      const list = page.getByTestId('settings-mcp-list')
      await expect(list).toContainText('0 个服务')
      await expect(page.getByTestId('settings-mcp-empty')).toBeVisible()
      const addBox = await add.boundingBox()
      await add.hover()
      expect(await add.boundingBox()).toEqual(addBox)
      ...
      await expect(add).toBeDisabled()
      expect(await add.boundingBox()).toEqual(addBox)
      await expect(page.getByTestId('settings-mcp-empty')).toHaveCount(0)
...
for (const theme of ['porcelain-blue', 'yao-stone', 'deep-plum', 'song-smoke']) {
  for (const width of [1166, 600]) {
    test('正式人物世界基础导航 ' + theme + ' ' + width, async ({ page }, testInfo) => {
      ...
      await expect(world.getByTestId('world-profile').getByRole('heading')).toHaveText('测试伙伴')
      await expect(world.getByTestId('world-profile')).not.toContainText('沉稳体贴的数字伙伴')
      await expect(page.locator('html')).toHaveAttribute('data-theme', theme)
      ...
      await expect(world).toContainText('还没有新的动态。')
      await expect(world).not.toContainText(/tick|Catch-up|派生/)
      ...
      await expect(last.locator('svg')).toHaveClass(/lucide-map-pin/)
...
test.describe('My Agent UI', () => {
  ...
  test('Playground 设置候选覆盖新的信息架构与隔离交互', async ({ page }) => {
    ...
    await candidate.getByTestId('settings-candidate-model-test').click()
    await expect(candidate.getByTestId('settings-candidate-model-status')).toContainText('连接测试通过：gpt-4o（样张）')
  ...
  for (const theme of ['yao-stone', 'porcelain-blue']) {
    for (const width of [1165, 520]) {
      test(`Playground 编程套餐完整交互 ${theme} ${width}`, async ({ page }, testInfo) => {
        ...
        await profile.getByRole('button', { name: /^获取 .* 已有模型$/ }).click()
  ...
  for (const theme of ['yao-stone', 'porcelain-blue']) {
    for (const width of [1165, 520]) {
      test(`Playground 连接卡片头部操作与模型候选 ${theme} ${width}`, async ({ page }, testInfo) => {
        ...
        const fetch = header.getByRole('button', { name: /^获取 .* 已有模型$/ })
        const testConnection = header.getByRole('button', { name: /^测试连接 / })
        ...
        await fetch.click()
        await expect(header.getByTestId('settings-candidate-fetch-models-connection-openai-0')).toBeDisabled()
        await expect(input).toBeVisible()
  ...
  for (const theme of ['porcelain-blue', 'yao-stone']) {
    for (const width of [1166, 600]) {
      test(`Playground 通讯录共享呈现与隔离 ${theme} ${width}`, async ({ page }, testInfo) => {
        ...
        await expect(panel.getByRole('button', { name: '刷新通讯录' })).toHaveCount(0)
        await expect(panel.getByRole('button', { name: '关闭通讯录' })).toHaveCount(0)
  ...
  test('Playground 当前主角贯穿 Chat、人物世界六个生活面', async ({ page }) => {
    ...
    await expect(page.getByTestId('playground-moments-profile')).toContainText('敏锐松弛的同行者')
    await expect(page.getByTestId('playground-moments-profile')).not.toContainText('沉稳体贴的数字伙伴')
  ...
  for (const theme of ['porcelain-blue', 'yao-stone', 'song-smoke', 'deep-plum']) {
    for (const width of [1166, 600]) {
      test(`正式记忆四类搜索与独立草稿 ${theme} ${width}`, async ({ page }, testInfo) => {
        ...
        await expect(page.getByTestId('settings-mcp-add')).toHaveCount(0)
