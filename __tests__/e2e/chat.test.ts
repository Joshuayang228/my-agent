...
test.describe('My Agent UI', () => {
  ...
  test('Playground 设置候选覆盖新的信息架构与隔离交互', async ({ page }) => {
    ...
    await expect(candidate.getByTestId('settings-candidate-model-add-form')).toContainText('编辑连接')
    await expect(candidate.getByLabel('连接名称', { exact: true })).toHaveValue('OpenAI 主账号')
    await expect(candidate.getByLabel('官方服务商', { exact: true })).toBeVisible()
    await candidate.getByLabel('连接名称', { exact: true }).fill('OpenAI 办公账号')
    await candidate.getByRole('button', { name: '保存连接', exact: true }).click()
