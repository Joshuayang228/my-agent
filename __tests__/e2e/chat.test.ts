...
test.describe('My Agent UI', () => {
  ...
  test('Playground 基础与产品体验分层且可独立验收', async ({ page }) => {
    ...
    for (const tab of ['按钮', '输入与表单', '标签与选择', '弹层与菜单', '状态与反馈', '开发基础']) {
    ...
    await page.getByRole('tab', { name: '开发基础', exact: true }).click()
    ...
    await page.getByRole('tab', { name: '弹层与菜单', exact: true }).click()
    ...
    await expect(page.getByTestId('motion-samples')).toBeVisible()
    ...
    await expect(iconSizeControls.getByRole('button', { name: '12px', exact: true })).toBeVisible()
    await expect(iconSizeControls.getByRole('button', { name: '20px', exact: true })).toBeVisible()
