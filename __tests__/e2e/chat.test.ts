...
test.describe('My Agent UI', () => {
  ...
  test('Playground 设置候选覆盖新的信息架构与隔离交互', async ({ page }) => {
    ...
    await candidate.getByTestId('settings-candidate-rules-create-toggle').click()
    await expect(candidate.getByText('自己指定某类操作：允许、需要确认，或直接拒绝。', { exact: true })).not.toBeVisible()
