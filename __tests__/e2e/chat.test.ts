...
test.describe('My Agent UI', () => {
  ...
  test('Playground 基础与产品体验分层且可独立验收', async ({ page }) => {
    ...
    await expect(chatDependencies).toContainText('体验组成')
    await expect(chatDependencies.getByTestId('experience-parts')).toContainText('会话导航')
    ...
    await expect(workspaceDependencies).toContainText('体验组成')
    await expect(workspaceDependencies.getByTestId('experience-parts')).toContainText('文件')
