...
describe('Provider 生产资产目录', () => {
  ...
  it('Provider 入口使用唯一注册表，Settings 与 Chat 不再维护平行数组', () => {
    ...
    expect(appSource).not.toContain('QUICK_PROVIDER_ENTRIES')
    expect(appSource).not.toContain("settings.set('llmBaseUrl'")
    expect(appSource).toContain("modelLabel={currentModel || '未配置模型'}")
    const composerSource = readFileSync('src/components/chat/ChatComposer.tsx', 'utf-8')
    expect(composerSource).toContain('data-testid="chat-current-model"')
    expect(composerSource).toContain('title={modelLabel}>{modelLabel}</span>')
