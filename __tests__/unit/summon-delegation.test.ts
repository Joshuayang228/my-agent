...
describe('summon-delegation (M26-G2)', () => {
  ...
  it('worker addon 仅 summon 注入，禁止装人设/推生活', () => {
    ...
    expect(addon).toMatch(/任务工/)
    expect(addon).toMatch(/不是该伙伴角色/)
    expect(addon).toMatch(/生活世界|动态|成长/)
  ...
  it('buildSubAgentSystemPrompt 在 summon 下带边界段', () => {
    ...
    expect(main).not.toMatch(/召唤 × multi-agent/)
    expect(summon).toMatch(/召唤 × multi-agent/)
    expect(summon).toMatch(/任务工/)
  })
})
