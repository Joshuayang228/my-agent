...
describe('playground catalog', () => {
  ...
  it('保留 UI 控件的细粒度状态故事', () => {
    expect(UI_CONTROLS_SUBTABS.map((tab) => tab.id)).toEqual([
      'action-button', 'buttons', 'inputs', 'icon-button', 'card', 'badge', 'tag', 'tabs', 'segmented-control', 'select', 'combobox', 'form-field', 'checkbox', 'switch', 'dialog', 'popover', 'dropdown-menu', 'workspace-tool-menu', 'command', 'context-menu', 'tooltip', 'empty', 'confirm', 'confirm-panel', 'feedback', 'toast', 'spinner', 'skeleton', 'progress', 'tool-cards', 'markdown', 'asset-table', 'file-tree', 'diff-viewer', 'scroll-area', 'resize-handle', 'divider', 'memory-chips', 'icons',
    ])
  })
})
